...
use crate::{
    domain::{AccountKey, ExchangeId, is_canonical_trading_account_id},
...
/// Holds the machine-wide scope lock for the complete lifetime of one Stage 7 writer entry.
...
pub(super) fn acquire(
    ...
    #[cfg(test)]
    let registry_root = artifacts_root.join(".stage7_writer_registry_test");
    #[cfg(not(test))]
...
fn account_key(scope: &WriterScope) -> Result<AccountKey, Stage7WriterRegistryError> {
    ...
    AccountKey::new(exchange, scope.account.clone()).map_err(|_| Stage7WriterRegistryError::Scope)
}
...
pub(super) enum Stage7WriterRegistryError {
