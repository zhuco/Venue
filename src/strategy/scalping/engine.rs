...
use crate::{
    ...
    strategy::scalping::{
        BlockingReason, CandidateEvidence, CandidatePreparation, Direction, EntryStyle,
        EpisodeProjection, EpisodeState, ExitTemplate, Expert, ExposureState, MarketRegime,
        NoopReason, ProtectionState, RiskGate, SafetyProjection, ScalpingCheckpoint,
...
pub struct ScalpingStrategy {
    ...
    state: ScalpingState,
    episode: Option<EpisodeProjection>,
    last_frame_generation: Option<u64>,
    last_watermark_ms: Option<u64>,
    cursors: BTreeMap<String, crate::indicator::SourceCursor>,
...
impl ScalpingStrategy {
    ...
    pub fn restore(
        ...
        if checkpoint.schema_version != crate::strategy::scalping::SCALPING_CHECKPOINT_SCHEMA
    ...
    pub fn checkpoint(&self) -> ScalpingCheckpoint {
        ScalpingCheckpoint {
            schema_version: crate::strategy::scalping::SCALPING_CHECKPOINT_SCHEMA,
    ...
    fn intent(
        ...
        Ok(SemanticIntent {
            ...
            risk_plan: crate::strategy::scalping::RiskPlan {
            ...
            reference_price: if self.binding.parameter_release_id
                == crate::strategy::scalping::PHASE8_ATR14_PARAMETER_RELEASE_ID
            {
                        ...
                        .contains(&EntryStyle::MarketableLimit) =>
            {
                ...
                let breakout_matches = frame.breakout.as_ref().is_some_and(|breakout| {
                    matches!(
                        ...
                        (crate::indicator::BreakoutDirection::Long, Direction::Long)
                            | (crate::indicator::BreakoutDirection::Short, Direction::Short)
                    )
                });
                if !breakout_matches {
                    return Vec::new();
    ...
    fn phase8_shock_reversal_enabled(&self) -> bool {
        self.binding.parameter_release_id
            == crate::strategy::scalping::PHASE8_ATR14_PARAMETER_RELEASE_ID
    }
...
pub(super) fn digest_strategy_binding(binding: &StrategyBinding) -> String {
    binding.digest()
}
...
fn opportunity_identity(
    ...
    expert: Expert,
    entry_style: EntryStyle,
) -> Result<(String, Option<crate::strategy::scalping::BreakoutCursor>), ScalpingError> {
    ...
    Ok((
        ...
        .to_lowercase(),
        Some(crate::strategy::scalping::BreakoutCursor {
