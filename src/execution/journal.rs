...
impl CommandJournal {
        ...
        for receipt in read_all(&path)? {
            ...
            if let Some(previous) = receipts.get(&command_id) {
                ...
                if let Some(client_id) = client_id {
                    if client_ids.insert(client_id, command_id.clone()).is_some() {
    ...
    pub(crate) fn commands(&self) -> impl Iterator<Item = &ExecutionCommand> {
...
fn command_hash(command: &ExecutionCommand) -> Result<String, CommandJournalError> {
    let payload = serde_json::to_vec(command).map_err(CommandJournalError::Encode)?;
