...
pub(crate) fn list_row(
    ...
    let selected = model.execution.inventory_mm.selected.as_deref() == Some(&item.instance_id);
    if ui
        .selectable_label(selected, format!("MM · {}", item.config.symbol))
        .clicked()
    {
        select(model, item);
    }
    ...
    ui.label(&credential.label)
        .on_hover_text(&item.trading_account_id);
    if ui.link(item.config.symbol.to_string()).clicked() {
        model.select_symbol(item.config.symbol.to_string());
        model.follow_latest_requested = true;
    }
    let color = match item.state {
        InventoryMmState::Running => theme::BUY,
        InventoryMmState::Stopped => theme::TEXT_SECONDARY,
        _ => theme::WARNING,
    };
    ui.colored_label(color, state_label(item.state));
    ui.monospace(format!("单笔 {}", item.config.order_notional))
        .on_hover_text(format!(
            "净库存上限 {} · 单腿上限 {} · 总库存上限 {}",
            item.config.max_net_notional,
            item.config.max_leg_notional,
            item.config.max_gross_notional
        ));
    ui.monospace(item.revision.to_string());
    if let Some(code) = item.attention.as_deref() {
        ui.colored_label(theme::WARNING, attention_text(code))
            .on_hover_text(code);
    } else {
        ui.weak(if item.last_quote_ms.is_some() {
            "已提交报价"
        } else {
            "尚未报价"
        })
        .on_hover_text("报价提交不代表交易所挂单或成交；管理窗口可查看配置与状态。");
    }
    ui.horizontal(|ui| {
...
fn select(model: &mut AppModel, item: &InventoryMmInstance) {
...
fn attention_text(code: &str) -> &str {
    match code {
        "private_facts_unavailable" => "私有数据不可用",
        "read_recovery_timeout" => "行情或风险数据恢复超时",
        "position_mode_changed" => "持仓模式已变化",
        "unexpected_conditional_orders" => "存在非预期条件单",
        "command_reconcile_required" => "订单待签名对账",
        "facts_or_planner_unavailable" => "行情或风控事实不可用",
        "account_loss_limit" => "触发账户亏损上限",
        "account_drawdown_limit" => "触发账户回撤上限",
        "planner_halted" => "风控已暂停报价",
        _ => code,
    }
}
...
pub(crate) fn show(
    ...
    if let Some(item) = item {
        ...
        egui::Window::new("独立库存做市管理").open(&mut open).resizable(true).vscroll(true).show(ui.ctx(), |ui| {
            ...
            if let Some(code) = item.attention.as_deref() {
                ui.colored_label(theme::WARNING, attention_text(code)).on_hover_text(code);
                ui.weak("确认原因已解除后，先停止并等待撤单确认，再重新签名预检和启动。");
            }
