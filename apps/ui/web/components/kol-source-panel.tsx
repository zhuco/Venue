...
export function KolSourcePanel({ csrf, credentials, onSource, disabled, children }: { csrf: string; credentials: Credential[]; onSource: (account: string | null) => void; disabled: boolean; children: ReactNode }) {
  ...
  return <section className="panel" aria-label="带单账户"><h2>带单账户（只能选择一个）</h2><a href="/help/kol#source" target="_blank" rel="noreferrer">查看设置步骤</a>
    ...
    <div className="heading"><p>当前带单账户：<strong>{current?.label ?? source?.trading_account_id ?? "尚未指定"}</strong></p><fieldset disabled={busy}>{children}</fieldset></div>
    <p className="muted">点击启用后，符合条件的新委托将同步到跟单账户。停止只撤销程序同步挂单，已有仓位不会自动平仓。</p>
    ...
    {source && !source.can_change ? <p className="muted">已有带单机器人或跟单关系，当前带单账户已锁定，不能直接更换。</p> : <form onSubmit={async event => {
      event.preventDefault(); if (!source || busy || disabled) return;
    ...
    }}><label>指定带单账户<select required value={choice} disabled={!source || busy || disabled} onChange={event => { setChoice(event.target.value); setSaved(false); }}><option value="">选择本人已验证的账户</option>{credentials.filter(c => c.verification === "verified" && c.dual_position).map(c => <option key={c.credential_id} value={c.credential_id}>{c.label}</option>)}</select></label><div className="buttons"><button disabled={!source || busy || disabled || !choice}>{busy ? "正在保存…" : "保存带单账户"}</button></div></form>}
    <p className="muted">此设置对所有登录端生效。验证通过并指定账户后自动开通带单，无需额外审批；保存不会启动带单。</p>
    <button disabled={busy || disabled} onClick={() => { setError(""); setSaved(false); void refresh(); }}>刷新带单账户</button>
  </section>;
}
