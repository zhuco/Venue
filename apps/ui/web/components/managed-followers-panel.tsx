    ...
    <dialog ref={dialog} className="managed-dialog" aria-labelledby="managed-title" onCancel={event => { event.preventDefault(); close(); }}>
      ...
      <form onSubmit={event => void save(event)}>
        <div className="managed-rows">{rows.map((row, index) => <fieldset key={row.id} disabled={busy || row.status !== "editing"} className="managed-row"><legend>账户 {index + 1}{row.status === "saved" ? " · 已保存" : ""}</legend>
          ...
          <label>API Secret<input aria-label={`账户 ${index + 1} API Secret`} type="password" value={row.secret} onChange={e => edit(row.id,"secret",e.target.value)} required minLength={16} maxLength={256} pattern="[A-Za-z0-9]+" autoComplete="off" spellCheck={false} /></label>
          <label>跟单方式<select aria-label={`账户 ${index + 1} 跟单方式`} value={row.sizing} onChange={e => edit(row.id,"sizing",e.target.value)}><option value="proportional">按比例</option><option value="fixed_notional">按单笔金额</option></select></label>
          {row.sizing === "proportional" ? <label>比例倍数<input aria-label={`账户 ${index + 1} 比例倍数`} inputMode="decimal" value={row.multiplier} onChange={e => edit(row.id,"multiplier",e.target.value)} required /></label> : <label>每笔金额（报价币）<input aria-label={`账户 ${index + 1} 每笔金额`} inputMode="decimal" value={row.fixedNotional} onChange={e => edit(row.id,"fixedNotional",e.target.value)} required /></label>}</div>
          <p className="muted">小于交易所最小数量或名义额时，开仓数量按步长向上进位，并计入总额度。</p>
