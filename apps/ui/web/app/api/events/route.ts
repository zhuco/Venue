...
import { controlHeaders, controlOrigin, getSession, noStore } from "@/lib/server";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const upstream = await fetch(target, { signal: abort.signal, cache: "no-store", redirect: "error", headers: controlHeaders({ "Last-Event-ID": after }) });
    clearTimeout(headersDeadline);
