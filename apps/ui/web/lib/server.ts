...
/** Only a server-configured Control user session may authenticate the fixed upstream. */
export function controlHeaders(initial?: HeadersInit): Headers {
...
export async function control(path: ControlPath, init?: RequestInit): Promise<Response> {
  ...
  try {
    ...
    const headers = controlHeaders(init?.headers);
    headers.set("content-type", "application/json");
    const upstream = await fetch(`${origin}${path}`, { ...init, signal, cache: "no-store", redirect: "error", headers });
    const responseHeaders = new Headers(upstream.headers); responseHeaders.set("x-venue-bff-control-ms", (performance.now() - started).toFixed(3));
