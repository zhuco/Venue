...
```powershell
./scripts/Invoke-VenueBuild.ps1 -CargoArguments @('build','--locked','-p','venue-node','--no-default-features','--features','bybit','--bin','venue-node-bybit')
```
...
[Build-VenueUbuntu](../../scripts/BUILD_POLICY.md)，不另开 target。
