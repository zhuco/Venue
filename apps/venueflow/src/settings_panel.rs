use eframe::egui::{self, Align2, RichText, Stroke};
...
impl SettingsPanelState {
    pub fn focus_indicators(&mut self) {
        self.tab = SettingsTab::Indicators;
    }
}
...
pub fn show(
    ...
    .show(context, |ui| {
        ui.set_min_size(egui::vec2(800.0, 530.0));
        ui.horizontal(|ui| {
            tab_button(
                ui,
                &mut state.tab,
                SettingsTab::Indicators,
                label(language, "指标设置", "Indicators"),
            );
            tab_button(
                ui,
                &mut state.tab,
                SettingsTab::General,
                label(language, "通用设置", "General"),
            );
        });
        ui.separator();

        match state.tab {
            SettingsTab::Indicators => indicator_settings(ui, state, language),
...
fn indicator_settings(ui: &mut egui::Ui, state: &mut SettingsPanelState, language: Language) {
    ...
    ui.horizontal(|ui| {
        ui.allocate_ui_with_layout(
            egui::vec2(190.0, 438.0),
            egui::Layout::top_down(egui::Align::Min),
            |ui| {
                ui.colored_label(theme::TEXT_SECONDARY, label(language, "主图", "Main chart"));
                for (kind, name) in [
                    ...
                    indicator_button(ui, &mut state.indicator, kind, name);
                }
                ui.add_space(10.0);
                ui.colored_label(theme::TEXT_SECONDARY, label(language, "副图", "Sub-chart"));
                for (kind, name) in [
                ...
                if let Some(draft) = state.draft.as_mut() {
                    ui.colored_label(
                        ...
                        format!(
                            "{}: {}",
                            label(language, "已启用", "Enabled"),
                            draft.enabled_study_count()
                        ),
                    );
                    ui.checkbox(
                        &mut draft.show_volume,
                        label(language, "显示成交量", "Show volume"),
                    );
                    ui.horizontal(|ui| {
                        ui.label(label(language, "图表字号", "Chart font"));
                        ui.add(
                            egui::DragValue::new(&mut draft.chart_text_size)
                                .range(9..=16)
                                .suffix(" px"),
                        );
                    });
...
fn indicator_button(
    ...
    if ui
        .add_sized(
            [176.0, 34.0],
            egui::Button::selectable(*selected == kind, RichText::new(name).strong()),
        )
        .clicked()
    {
        *selected = kind;
...
fn indicator_editor(
    ...
    let title = match kind {
        IndicatorKind::Sma => "SMA · Simple Moving Average",
        IndicatorKind::Ema => "EMA · Exponential Moving Average",
        IndicatorKind::Bollinger => "BOLL · Bollinger Bands",
        IndicatorKind::Vwap => "VWAP · Volume Weighted Average Price",
        IndicatorKind::Rsi => "RSI · Relative Strength Index",
        IndicatorKind::Macd => "MACD · Moving Average Convergence Divergence",
        IndicatorKind::Atr => "ATR · Average True Range",
    };
    ...
    ui.colored_label(
        ...
        label(
            language,
            "参数仅作用于本地公共行情图表，不影响策略或服务器计算。",
            "Parameters only affect the local public chart, never strategy or server state.",
        ),
    );
    ui.add_space(14.0);
    ...
    match kind {
        IndicatorKind::Sma => {
            period_editor(ui, language, &mut settings.sma_period);
            style_editor(ui, language, &mut settings.sma, false);
        }
        IndicatorKind::Ema => {
            period_editor(ui, language, &mut settings.ema_period);
            style_editor(ui, language, &mut settings.ema, false);
        }
        IndicatorKind::Bollinger => {
            period_editor(ui, language, &mut settings.bollinger_period);
            ui.horizontal(|ui| {
        ...
        IndicatorKind::Rsi => {
            period_editor(ui, language, &mut settings.rsi_period);
        ...
        IndicatorKind::Atr => {
            period_editor(ui, language, &mut settings.atr_period);
            style_editor(ui, language, &mut settings.atr, false);
        }
    }
}

fn period_editor(ui: &mut egui::Ui, language: Language, period: &mut u32) {
    ui.horizontal(|ui| {
        ui.label(label(language, "周期", "Period"));
...
fn style_editor(
    ...
    ui.checkbox(
        &mut style.enabled,
        label(language, "显示指标", "Show indicator"),
    );
    ui.add_space(8.0);
    egui::Grid::new("indicator-style-editor")
        ...
        .spacing([18.0, 12.0])
        .show(ui, |ui| {
            ui.label(label(language, "主线颜色", "Primary color"));
            ...
            ui.label(label(language, "线宽", "Line width"));
            ui.add(
                egui::Slider::new(&mut style.line_width_tenths, 5..=40)
