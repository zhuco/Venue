...
test("Control authorization uses only the server user session, never caller or Node credentials", async () => {
  ...
  const priorNode = process.env.VENUE_CONTROL_NODE_TOKEN;
  const original = globalThis.fetch;
  try {
    delete process.env.VENUE_WEB_CONTROL_SESSION_TOKEN;
    process.env.VENUE_CONTROL_NODE_TOKEN = "node-fixture-token";
    assert.equal(controlHeaders({ authorization: "Bearer caller" }).has("authorization"), false);
    process.env.VENUE_WEB_CONTROL_SESSION_TOKEN = "control-user-fixture-token";
    const headers = controlHeaders(new Headers({ authorization: "Bearer caller", "last-event-id": "7" }));
    assert.equal(headers.get("authorization"), "Bearer control-user-fixture-token");
    assert.equal(headers.get("last-event-id"), "7");
    globalThis.fetch = async (_url, init) => {
      assert.equal(new Headers(init?.headers).get("authorization"), "Bearer control-user-fixture-token");
      assert.equal(init?.redirect, "error");
      return Response.json({ ok: true });
    };
    const response = await control("/v2/ui/snapshot", { headers: { authorization: "Bearer caller" } });
    assert.equal(response.status, 200);
    assert.equal(response.headers.has("authorization"), false);
    assert.deepEqual(await response.json(), { ok: true });
  } finally {
    globalThis.fetch = original;
    if (prior === undefined) delete process.env.VENUE_WEB_CONTROL_SESSION_TOKEN; else process.env.VENUE_WEB_CONTROL_SESSION_TOKEN = prior;
    if (priorNode === undefined) delete process.env.VENUE_CONTROL_NODE_TOKEN; else process.env.VENUE_CONTROL_NODE_TOKEN = priorNode;
  }
});
