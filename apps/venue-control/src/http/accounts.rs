...
use venue_control_protocol::{
    ...
    support_martingale::{
        ...
        SUPPORT_MARTINGALE_LIFECYCLE_PATH, SUPPORT_MARTINGALE_PREFLIGHT_PATH,
...
async fn account_request(
    ...
    match (request.method, path) {
        ...
        (Method::Post, SUPPORT_MARTINGALE_PREFLIGHT_PATH) => encode(
            &accounts
                .support_martingale_preflight(
                    &principal,
                    decode::<SupportMartingalePreflightRequest>(&request.body)?,
                    now,
                )
                .await?,
        ),
