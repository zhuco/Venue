...
use venue_control_protocol::{
    ...
    accounts::*,
    kol::{
        FollowLifecycleRequest, FollowSettingsUpsertRequest, KOL_FOLLOW_LIFECYCLE_PATH,
        KOL_FOLLOW_SETTINGS_PATH, KOL_PROFILE_PATH, KolProfileUpdateRequest,
    },
};
    ...
    R: ControlRepository + AccountDeliveryRepository + CopyRelationRepository + 'static,
{
    ...
    if path.starts_with("/v2/account/")
        || matches!(
            path,
            KOL_PROFILE_PATH | KOL_FOLLOW_SETTINGS_PATH | KOL_FOLLOW_LIFECYCLE_PATH
        )
    {
        let Some(accounts) = accounts else {
            return account_error(stream, AccountErrorCode::Unavailable).await;
