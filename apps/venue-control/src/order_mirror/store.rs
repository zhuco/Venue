...
pub async fn run_order_mirror(
    ...
    loop {
        ...
        let relations:Vec<String>=match sqlx::query_scalar("SELECT r.relation_id FROM venue_kol_follow_relations r WHERE (r.relation_state='active' AND EXISTS(SELECT 1 FROM venue_leader_bots b WHERE b.owner_user_id=r.kol_user_id AND b.bot_state<>'stopped')) OR EXISTS(SELECT 1 FROM venue_order_mirrors m WHERE m.relation_id=r.relation_id AND m.mirror_state NOT IN ('terminal','blocked')) ORDER BY r.relation_id")
