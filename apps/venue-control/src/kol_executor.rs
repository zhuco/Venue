...
/// Bounded union of followers, KOL leaders, Grid accounts and active terminal subscriptions.
pub const MAX_ACTIVE_EXECUTOR_ACCOUNTS: usize = 232;
    ...
    pub fn enqueue(
        ...
        if trading_account_id.is_empty()
            ...
            || (!self.accounts.contains_key(&trading_account_id)
                && self.accounts.len() >= MAX_ACTIVE_EXECUTOR_ACCOUNTS)
        {
            return Err(BinanceCommandLedgerError::Conflict);
        }
        let queue = self
...
mod tests {
    ...
    #[test]
    fn scheduler_reserves_capacity_beyond_followers_for_shared_executor_accounts()
    -> Result<(), Box<dyn std::error::Error>> {
        let mut scheduler = AccountSerialScheduler::new(1);
        for value in 0..MAX_ACTIVE_EXECUTOR_ACCOUNTS {
            scheduler.enqueue(format!("account-{value:03}"), value)?;
        }
        assert_eq!(
            scheduler
                .enqueue("account-over-limit".into(), MAX_ACTIVE_EXECUTOR_ACCOUNTS)
                .err(),
            Some(BinanceCommandLedgerError::Conflict)
        );
        let first = scheduler.claim_next().ok_or("first account absent")?;
        scheduler.settle(&first.trading_account_id)?;
        scheduler.enqueue(
            "account-after-settlement".into(),
            MAX_ACTIVE_EXECUTOR_ACCOUNTS,
        )?;
        Ok(())
    }
    ...
    #[test]
    fn grid_claims_are_gated_by_the_durable_predecessor_chain() -> Result<(), &'static str> {
        let source = include_str!("kol_executor.rs");
        let claim = source
