...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct SupportMartingaleDatabasePreflight {
    pub credential_verified: bool,
    pub account_exclusive: bool,
}
...
impl SupportMartingaleStore {
    ...
    pub async fn list(
        ...
        let rows = sqlx::query("SELECT instance_id,execution_venue,trading_account_id,lifecycle,health,revision,config,(SELECT count(*) FROM venue_support_martingale_symbol_states s WHERE s.instance_id=i.instance_id) AS symbol_count,(SELECT min(health_reason) FROM venue_support_martingale_symbol_states s WHERE s.instance_id=i.instance_id AND health_reason IS NOT NULL) AS health_reason,reserved_budget::text AS reserved_budget FROM venue_support_martingale_instances i WHERE owner_user_id=$1 ORDER BY instance_id").bind(owner).fetch_all(&self.pool).await.map_err(|_| SupportMartingaleStoreError::Unavailable)?;
        rows.into_iter()
            .map(|r| {
    ...
    pub async fn lifecycle(
        ...
        if matches!(
            ...
            if record.health != SupportMartingaleHealth::Healthy {
                return Err(SupportMartingaleStoreError::Conflict);
            }
    ...
    pub async fn database_preflight(
        &self,
        owner: &str,
        instance_id: &str,
        expected_revision: u64,
    ) -> Result<SupportMartingaleDatabasePreflight, SupportMartingaleStoreError> {
        let record = self.get(owner, instance_id).await?;
        if record.revision != expected_revision
            || record.lifecycle != SupportMartingaleLifecycle::Stopped
            || record.health != SupportMartingaleHealth::Healthy
        {
            return Err(SupportMartingaleStoreError::Conflict);
        }
        let mut tx = self
            ...
            .map_err(|_| SupportMartingaleStoreError::Unavailable)?;
        lock_account(&mut tx, &record.trading_account_id).await?;
        let credential_verified: bool = sqlx::query_scalar(
            "SELECT EXISTS(SELECT 1 FROM venue_api_credentials WHERE credential_id=$1 AND user_id=$2 AND trading_account_id=$3 AND venue=$4 AND deleted_ms IS NULL AND verification_json->>'verification'='verified' AND verification_json->>'strategy_execution'='true')",
        )
        .bind(&record.credential_id)
        .bind(owner)
        .bind(&record.trading_account_id)
        .bind(record.execution_venue.as_str())
        .fetch_one(&mut *tx)
        .await
        .map_err(|_| SupportMartingaleStoreError::Unavailable)?;
        let account_exclusive: bool = sqlx::query_scalar(
            "SELECT NOT (EXISTS(SELECT 1 FROM venue_strategy_grids WHERE trading_account_id=$1 AND lifecycle IN ('running','pausing','paused','stopping','resetting')) OR EXISTS(SELECT 1 FROM venue_control_strategy_scopes WHERE trading_account_id=$1) OR EXISTS(SELECT 1 FROM venue_binance_commands WHERE trading_account_id=$1 AND command_state IN ('pending','sending','accepted','reconcile_required')) OR EXISTS(SELECT 1 FROM venue_support_martingale_instances WHERE trading_account_id=$1 AND instance_id<>$2 AND lifecycle IN ('running','entry_paused','increase_paused','draining')))"
        )
        .bind(&record.trading_account_id)
        .bind(instance_id)
        .fetch_one(&mut *tx)
        ...
        tx.rollback()
            .await
            .map_err(|_| SupportMartingaleStoreError::Unavailable)?;
        Ok(SupportMartingaleDatabasePreflight {
            credential_verified,
            account_exclusive,
        })
    }
