...
- 冻结 `apps/venueflow/**`，不开发交易终端、桌面/Web UI、图表、交互或 UI E2E；
...
1. **Goal 0 必须先独占完成。** 它冻结 LIVE-only 公共契约，其他 Goal 不得在旧 `TEST | LIVE` DTO 上继续开发。
2. Goal 1–7 在 Goal 0 合并后可并行；当前四并发槽建议分两批，每批最多三个子任务，主任务保留一个整合槽。
3. Goal 8 依赖 Goal 1–5，涉及生产组合与接管，三个交易所内部固定 Gate.io -> Bitget -> Binance，禁止并行 mutation。
4. Goal 9 最后独占共享 Cargo、lock、长期文档、全仓门禁、提交和推送。
5. 推荐复杂实现使用 `gpt-5.6-sol`、`high`；每个 Goal 使用独立分支/worktree 或明确文件租约。
6. 子任务只显式暂存自有路径，禁止 `git add -A`；发现共享文件需求先停下协调。
7. 依赖表是优先基线；当前 Goal 存在可验证缺口时可新增依赖，但必须先查 workspace/lock、说明理由、加入真实调用与专项测试，
   不得为未来功能预装或无理由复制同类栈。
8. Cargo 构建产物统一位于 `G:\Build\Venue`；并行任务在其下使用按 Goal/PID 隔离的子目录，禁止写入仓库 `target/`。

共享文件租约：

| 路径 | 独占 Goal |
|---|---|
| 根 `Cargo.toml`、`Cargo.lock` | Goal 9；Goal 0 如确需删依赖须先列明 |
| `CODEMAP.md`、`ARCHITECTURE.md`、`GRID_RUNTIME_REFACTOR.md`、本文 | Goal 9，Goal 0 只更新 LIVE-only 契约 |
| `.github/workflows/**` | Goal 6 |
| `apps/venueflow/**` | 全程冻结 |
| `bak/**` | 全程冻结 |

## 5. Goal 0：收敛为 LIVE-only 公共契约

**目标**：删除所有生产运行时 `TEST` 分支、测试网/demo endpoint 和 TEST artifact scope；公共协议、配置、固定 binary、
Control/Copy scope 只接受精确 `LIVE`。离线测试继续使用 test fixture 类型或 mock transport，不得伪装成网关 mode。

**独占路径**：

- `crates/venue-gateway-api/**`
- `crates/venue-control-protocol/**`
- `apps/venue-node/src/bin/**` 中仅 mode/config 解析部分
- `apps/venue-control/src/**` 中仅 mode 校验与 TEST-only worker 开关
- `scripts/verify_gateway_candidate_contract.ps1`
- 与 mode 直接相关的非 UI 测试和示例配置

**必须证明**：

- 生产枚举、CLI、TOML、环境变量和 DTO 不再接受 `TEST`、testnet、demo 或隐式布尔模式；
- `LIVE` endpoint 不可配置为测试网，凭证/root 仍按 venue/account 精确隔离；
- 测试全部使用离线 fixture/mock，CI 零真实交易所网络和零真实 mutation；
- 不因为删除 `TEST` 而开放任何尚未获准的 production POST、capability、writer 或 dispatch。

**依赖**：无。**阻塞**：Goal 1–8。

## 6. Goal 1：Actor Applied 与 Runtime 权威桥接
...
**依赖**：Goal 0。**可并行**：Goal 2–7。

## 7. Goal 2：Binance 生产恢复证据桥
...
**依赖**：Goal 0 的固定 recovery DTO。**可并行**：Goal 1、3–7。

## 8. Goal 3：Gate.io 生产恢复证据桥
...
**依赖**：Goal 0。**可并行**：Goal 1–2、4–7。

## 9. Goal 4：Bitget 最终六面 Fold 与恢复证据桥
...
**依赖**：Goal 0。**可并行**：Goal 1–3、5–7。

## 10. Goal 5：Copy LIVE-only 耐久工作流闭合

**目标**：复用现有 Copy reducer 和 PostgreSQL 模型，去掉 TEST-only worker 限制，形成 LIVE-only 的 leader intent ->
...
**依赖**：Goal 0；Node 终态 Applied 联调依赖 Goal 1/8。**可并行**：Goal 1–4、6–7。

## 11. Goal 6：PostgreSQL 非跳过 CI 门禁
...
**依赖**：Goal 0。**可并行**：Goal 1–5、7。

## 12. Goal 7：指标到 Control 的只读生产投影
...
- `crates/venue-control-protocol/**` 中指标只读 DTO（必须基于 Goal 0 后契约）
...
**依赖**：Goal 0。**可并行**：Goal 1–6。

## 13. Goal 8：账户 Node 组合与三所 LIVE 单 writer 接管
...
另须证明：PostgreSQL CI 实际运行、全部 fixed binary 只接受 LIVE、构建和测试不连接真实交易所、`apps/venueflow/**` 与
`bak/**` 无改动、工作树 clean、HEAD 与目标远端一致。真实 Canary 收据单独保存为受保护 artifact，不作为普通测试日志处理。
...
在接口稳定、无外部凭证/交易所阻塞时，纯代码约 17–34 小时；真实三所 Canary 与稳定性观察另按每所独立窗口安排，
