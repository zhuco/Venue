...
- Ubuntu 产物默认在本机用 `scripts/Build-VenueUbuntu.ps1` 交叉编译后上传，不在 `45.77.253.180` 日常编译。专用根 `G:\Build\Venue\ubuntu` 保存源码快照、Zig 工具缓存和版本化产物；Cargo 复用已有 slot-2 与同一 guard。目标、工具版本、预检和上传边界见 `scripts/BUILD_POLICY.md`，不另开 Cargo target。
