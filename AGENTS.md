...
活动代码由根 `Cargo.toml` 声明的 Rust workspace（根 package、`apps/`、`crates/`）及独立 `apps/venue-web` npm 应用组成；长期说明统一在 `docs/`。当前唯一产品目标是 Binance KOL 跟单 MVP：邀请注册、登录、API Key 绑定与验证、KOL 交易终端、快速跟单和 KOL 落地页。初期全站不超过 5 个启用 KOL、200 个启用跟单账户；Grid、Gate.io、Bitget 及其余交易所迁移与 Scalping 暂缓。
