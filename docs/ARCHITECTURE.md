...
本文描述 alpha.27 的代码结构与当前发布范围；功能入口见 [CODEMAP](CODEMAP.md)，产品行为和验收以 [KOL MVP](KOL_COPY_MVP.md)、[带单同步](LEADER_ORDER_MIRROR.md) 和 [Grid 契约](GRID_RUNTIME_REFACTOR.md) 为准。部署状态须读取实际运行版本，不能由源码或标签推断。

## 1. 产品范围

独立多交易所策略的新增源码入口见 [MULTI_VENUE_EXECUTOR](MULTI_VENUE_EXECUTOR.md)：另外五所复用相同单例进程、PostgreSQL 账本与密文边界，协议留在 adapter。此扩展的本地验证与逐所实盘验收独立于下述 Binance KOL 发布范围。

当前新链面向 Binance Portfolio Margin UM 双向持仓账户，提供 KOL 普通限价挂单同步、桌面终端与事实驱动对冲网格。初期最多 5 个启用 KOL、200 个启用跟随账户；收费结算、策略广场、跨交易所跟单和多 Executor 分片不在本版范围。

alpha.27 中其他交易所 adapter、旧 Node、Actor 和本地 WAL 保留兼容用途，不因仍能编译而成为本版 KOL 执行入口。当前工作区另有独立多交易所策略执行开发，见第 8 节；未提交的改动不能当作已经发布的功能。
...
| `venue-executor-binance` | 单例锁、认证私流与签名恢复、挂单规划、Grid、账户调度、物理下单及对账 |
| `venue-leader-bot-admin` | 管理员迁移、带单授权及撤权；不提供公开授权 HTTP 接口，不执行交易 |
| Binance adapter | 签名、校时、规则、原生字段转换与规范订单/仓位事实 |
...
- `venue-strategies` 的 Grid Planner 只根据配置与规范事实计算目标，不接触凭证、数据库或交易所客户端。
- `venue-control` 的 Store 持久化业务状态，Runtime 编排任务，Exchange 层调用 adapter。
- `venue-gateway-binance` 封装交易所原生差异；其余 gateway 的冻结状态见第 8 节。
...
只同步机器人与关系启用后符合条件的新普通限价单，保留源价、方向、持仓腿及 GTC/PostOnly。定比和定额在每个跟随关系独立计算，按步长向下取量。主单部分成交不反复重挂子单；改单先确认旧子单终态并扣除自身累计成交，再建立替代单；主单结束撤销子单剩余量。

市价、Algo/条件单及主从仓位差异不触发追补。旧成交目标模型仅为历史和未决命令恢复保留，不能描述为新关系的复制方式。完整数量、限制与恢复规则统一维护在 [LEADER_ORDER_MIRROR](LEADER_ORDER_MIRROR.md)。

Grid 从配置、签名基线及连续认证私流生成目标订单；Planner 计算，Store 原子提交批次，Runtime 唤醒共享 Executor 补撤。正常热路径复用有效规则与时钟，异常转签名恢复；重启依赖 PostgreSQL 和交易所事实。具体调用链、批内顺序和拒单恢复见 [Grid 开发结构](GRID_STRATEGY_ARCHITECTURE.md) 与 [Grid 契约](GRID_RUNTIME_REFACTOR.md)。
...
当前开发中的 `multi_venue_*`、`multi_venue_grid/`、`durable_gateway`、迁移 `0035–0036` 与 `venue-strategy-admin` 为非 Binance 独立策略和网格复用同一 Executor、纯规划器和账户网络并发上限，不启动额外交易所进程。网格目标面、累计成交消费与命令原子入账，Net 账户显式选择方向。其新增调用与 alpha.27 的冻结旧链分开，契约和逐所验证见 [多交易所执行](MULTI_VENUE_EXECUTOR.md)；开发完成、发布和实盘验收分别记录。
...
- 冻结 Node CLI 见 [NODE](NODE.md)；恢复保护与删除门见 [GRID_RUNTIME_REFACTOR](GRID_RUNTIME_REFACTOR.md)。本次文档清理不改变这些兼容边界。
