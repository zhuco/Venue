...
Windows 将有效登录会话保存至系统凭据库，启动后向服务端验证身份。会话有效期仍为 12 小时；启用保存密码后，启动或会话到期时自动重新认证，临时网络失败间隔 30 秒重试，主动退出或认证被拒绝后停止自动登录。桌面按服务器地址和登录用户持久化执行凭证选择，重新登录时仅恢复仍可选择的本人绑定，并等待服务端确认后启用私有操作；本地偏好不构成授权。手动切换更新偏好，普通刷新不覆盖用户选择。API 表格按备注/掩码、交易所、账户类型、账户身份、验证、选择和删除排列。顶部选择器和底部状态栏使用同一选中凭证备注；账户连接状态统一读取 Executor 私有投影，不回退旧 Node。
