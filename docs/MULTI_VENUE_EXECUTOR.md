...
Bitget 普通 UTA 与原生带单组合可返回相同 UID，但专用带单 Key 的 UTA 资金、持仓与订单查询属于带单范围。账户身份按签名 UID 与交易范围区分：普通 UTA 保留原身份，显式带单凭证经带单权限及签名白名单验证后使用独立带单范围，不按 Key 指纹、标签或每次绑定随机值划分。当前每个 UID 只支持一个带单执行范围；同一带单组合的两把 Key 或换 Key 仍复用同一队列，不声明支持多个独立带单组合。普通网格与带单马丁可分别占用各自范围；同范围内仍执行策略互斥，不自动停旧策略或替换其凭证。
...
重建时仅在自有旧单全部签名确认终态、无账户未决命令且规划完成 resetting→running 后清除旧收敛计时；新一轮挂单独立计时。该边界保留连续失败次数，不能靠反复重建绕过失败阈值；单纯重试、部分撤单完成或未知结果不重置期限。

暂停/停止的触发原因在 pausing/stopping 撤单阶段及 paused/stopped 终态连续保留，不能因发出保护性撤单而清空；显式恢复按原生命周期入口清除门。
