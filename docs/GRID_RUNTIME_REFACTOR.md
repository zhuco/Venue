...
- Control poll 的 transport、timeout 或 HTTP 非成功状态持续按最多 5 秒退避，只推进自己的 deadline，不能因连续次数耗尽而退出账户 writer；退避期间仍运行同一账户的私有/公共 pump。响应协议、scope、耐久工件或 Actor 语义错误仍立即失败关闭。公共空闲等待最多 5ms，有积压则继续有界排空。该值不是端到端延迟保证：同步签名读取、HTTP、持久化及 reducer 耗时仍须单独测量和消除瓶颈。
...
账户级签名快照可以同时携带其他规范交易对的既有仓位；目标 Grid 的精确订单面只绑定本策略交易对，启动撤净与重建不得要求清掉或忽略无关仓位。非撤单批次仍从完整账户签名风险证据采集和估值，并执行既有逐子单准入；无关挂单、缺失 Owner 或订单形状漂移不能借此穿过目标订单面校验。
...
生产冷启动恢复私有游标还必须同时验证 Host 已核验的 Actor Applied 头与完整 `facts.jsonl` 尾部：每条事实的 journal 序号、header source sequence、PrivateAccount 来源、header 与规范私有事件均须一致有效；通常只有 Actor 游标精确命中已验证 facts 尾部时才同步 Runtime 与 PrivateRouter。生产 Node 必须先登记该账户全部配置 Actor，再安装各 Actor Applied 工件，避免用安装顺序猜 Owner。唯一允许的前写崩溃窗口是 Grid 的 facts 尾部领先 Actor：领先段必须全部是同一 Host 最新完整签名页中逐字节唯一匹配且在尾段内不重复的成交，并各自精确命中同一本 WAL 的唯一 Accepted native id 与已登记 Grid Owner；Host 只覆盖本次内存证据游标，启动恢复仍须把这些签名成交以新的连续 facts 序号重新持久化并写入 Actor checkpoint 后才可 Running。任一非成交、缺失、重复、Owner/WAL 不一致、重标记、断序或坏尾都失败关闭；不得改写或截断原 facts，也不得恢复或重投 mutation。
...
生命周期 delivery 在 poll、ACK、fsync 和 Actor 应用之后必须重新取时生成回执，不能复用 tick 起始时间；本地租期应覆盖 5 秒 HTTP 超时与持久化/签名读取预算，生产默认使用 30 秒但仍受协议 60 秒上限约束。Pause/Resume 的 Actor checkpoint 必须同时绑定 `request_id`、delivery identity 与规范请求摘要；租期后只能在最新 Runtime 验证 checkpoint 精确匹配且生命周期一致时返回 Reconciled。历史仅含 action 的 checkpoint、摘要错配、被后续 turn 覆盖或状态不一致均保持待人工对账，不重放 Actor，不伪造成功。
...
统一 Node 的 `production_resident/manual.rs` 已接入显式限价及自有手动挂单撤单；终端使用 `StrategyKind::Manual` 的 dormant actor，它不消费市场输入、不产生自动策略意图，也不得携带 Grid/Scalping 配置或 `copy_leader_capital`。Control 与 VenueFlow 只允许 Trade 精确命中该 kind。稳定 request ID、原始计划和数量上限存于同一 Actor replay 的可选 manual 字段，后续 checkpoint 必须保留该字段。恢复验证原绑定和配置，Reconcile 只读原 WAL 与签名事实，不重投；新 Accepted WAL 路由立即回填 Runtime，私流还必须同时匹配 adapter 的签名 private generation，并以 Runtime connection generation 写事实 journal。只有精确挂单或完整成交与仓位变化得到签名证明才返回完成。Copy 绑定及会影响 Grid desired 的撤单继续明确 Rejected，不能把部分 scope 撤单称为撤全部。此桥接不代表生产接管或所有账户能力已验收，也不得恢复旧 writer 旁路。
