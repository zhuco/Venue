...
认证成交私流负责去重并尽快唤醒计算，但不单独授予下单权。只有新鲜签名基线、连续私流序列和新鲜 BBO 同时成立时，运行时才可用成交增量推进内存投影而不为每笔成交同步等待完整签名 REST；任一条件失效即停止 mutation。签名订单和双向持仓仍是重启、补漏与纠偏事实，断线后重新获取签名快照，不能要求回放全部历史事件。
...
- 库存低于配置阈值时进入 `Replenishing`，先停止普通增险并撤本实例冲突订单，再通过统一命令账本补充明确的 Long/Short 库存；签名仓位确认后以补充成交价或新鲜 BBO 重新锚定；
...
- 私流增量只在上述新鲜门内唤醒和推进投影，不改变签名事实的重启权威性；私流断线、事件乱序或同时成交均关闭热路径，周期签名快照负责补漏并恢复收敛；
