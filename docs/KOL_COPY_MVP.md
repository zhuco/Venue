...
启用的 KOL 可添加已获账户所有者委托的 Binance API，填写标签、Key 和 Secret，单次对话框最多 10 个、每 KOL 最多 200 个保存账户。此功能与邀请注册分别管理：每个托管账户建立不可登录的内部用户，只在 `venue_kol_managed_followers` 固定归属，尚不创建邀请绑定、跟单关系或执行权限。原有跟随用户仍不能由 KOL 查看密钥或代为切换账户。
