...
- KOL 后台通过 `GET/POST /v2/kol/invite` 查看、随机生成或自定义邀请码，公开链接为 `/join/<invite_code>`。自定义为 6–64 位 ASCII 字母、数字、`-`、`_`，去除两端空白、区分大小写；全局 `code_hash` 唯一约束覆盖所有 KOL 及已停用历史码，不允许另一个 KOL 复用。每个 KOL 只有一个当前有效码，更换前须确认旧链接失效；冲突与失败在事务内回滚，旧码保持有效。
