...
托管账户验证成功后按已保存授权自动建立关系并申请激活，不要求第二次风险勾选。内部暂停态只承载空仓、无挂单、权限、账户归属和单 writer 的激活闸门；迁移 `0038` 保存授权。`/v2/kol/managed-followers/delete` 会先自动暂停关系并撤销程序子单，再要求签名零持仓/零挂单且无未决镜像或命令；满足后擦除凭证恢复材料，保留关系、命令和成交审计，不删除 Binance 子账户。
