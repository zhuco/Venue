...
每个托管账户独立保存定比或定额设置。定比保留主单数量 × 分配资金 / KOL 资本 × 倍率；定额以正数报价币金额除以源限价，不乘倍率，且不得超过单笔上限。两者均受数量步长、最低名义额、总风险和只减仓裁剪约束；不向上放大预算。源单修改后扣除其已有子单累计成交，部分成交不产生市价追补。

迁移 `0032` 将旧关系保留为定比；`0033` 以来源二选一及外键约束固定托管归属；`0034` 增加私有带单机器人目录、配置版本及单活动实例约束。保存设置在同事务校验 owner、创建托管绑定、暂停关系和幂等请求审计；凭证验证失败整笔回滚。相同 request ID 不同内容拒绝。停用 KOL 仍能暂停原关系，不能激活；内部托管用户不能使用普通登录跟单写入口。状态/设置/生命周期仅通过 owner-scoped 的 `/v2/kol/managed-followers/follow/{status,settings,lifecycle}` 开放；保存不启用交易。

源订单来自认证账户流及签名恢复，不要求使用 Venue 下单。同一被选定 KOL 账户从币安原生 App/Web 创建的普通 UM GTC/PostOnly 限价单也可准入，但必须晚于机器人启动和跟单激活、仍有剩余量、符合白名单和风险限制。市价、条件/Algo、启用前旧单不复制。
...
启用的 KOL 可添加已获账户所有者委托的 Binance API，填写标签、Key 和 Secret，单次对话框最多 10 个、每 KOL 最多 200 个保存账户。此功能与邀请注册分别管理：每个托管账户建立不可登录的内部用户，只在 `venue_managed_credentials` 固定归属，尚不创建邀请绑定、跟单关系或执行权限。原有跟随用户仍不能由 KOL 查看密钥或代为切换账户。
...
每个启用 KOL 在 MVP 中恰好绑定一个已验证的主交易账户和一个公开页面；同一主账户最多保存 20 条私有带单机器人配置，每条独立保存名称、说明和策略资金。跟随关系当前绑定 KOL 而非机器人，因此同一 KOL 最多一条带单机器人处于运行、排空或需处理状态。每个跟随用户只能归属一个 KOL，并且同一时间最多启用一个 Binance 跟随交易账户。用户可保存替换用凭证，但切换活动账户前必须暂停关系并重新通过启用门。所谓“绑定 KOL”是 Venue 中的关系，不把用户账户变成 KOL 可登录、可切换或可代操作的 Binance 子账户。
...
管理员按用户显式授权；默认无带单权限、不可创建、不可启动，但已有 KOL 资料须显示为“待管理员授权”。KOL 用本人的已验证主账户创建多条私有配置，创建后为停止态；只有停止态且无未决镜像命令时可编辑。启用时必须确认没有同 KOL 的其他非停止实例。用户邀请注册、API 权限和跟单激活仍独立验证。撤权禁止新增同步，并保留已有实例状态与停止操作。

已登录用户的带单访问响应始终返回其自身 KOL 资料状态；没有管理员授权时仍须显示“待管理员授权”，不得把带单入口静默隐藏或误导 KOL 使用普通跟随者设置。桌面机器人页以一个私有列表统一展示 Grid 和带单实例的名称、策略、主账户、状态、运行数据、版本和注意事项；不提供策略广场。状态未刷新或请求待确认时不得继续创建、编辑或启停。KOL 的托管账户按账户独立设置和启用，普通跟随者表单只对非 KOL 用户显示。
