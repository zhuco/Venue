...
每账户独立选择定比或定额跟单，迁移 `0032` 的旧记录默认定比。定比保留原数量比例公式；定额使用正数报价币名义金额除以限价、止损触发价或市价源单首次成交价，不再乘资金比例或倍数。两种模式的开仓数量低于交易所最低名义额时向上补足，达到最低额后按交易所步长向下取整且不超过计划金额；边界上若向下会失去合规性，则取最小合规数量。实际金额仍受账户总风险上限和交易所最大数量限制。已有子单成交先扣减；只减仓数量仍向下裁剪，不补足开仓最低额。主账户以不少于原开仓 99% 的反向限价单退出一个曾因最低额补足的来源仓位时，子账户平掉该实际补足数量，再按当前可减仓数量和已有平仓委托裁剪。命令持久化取整策略，旧命令回读保留原规则，详见 [人工带单数量契约](LEADER_ORDER_MIRROR.md#同步规则)。改变运行中参数仍须先完成撤单和对账。
