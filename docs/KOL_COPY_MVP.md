...
迁移 `0032` 将旧关系保留为定比；`0033` 以来源二选一及外键约束固定托管归属。保存设置在同事务校验 owner、创建托管绑定、暂停关系和幂等请求审计；凭证验证失败整笔回滚。相同 request ID 不同内容拒绝。停用 KOL 仍能暂停原关系，不能激活；内部托管用户不能使用普通登录跟单写入口。状态/设置/生命周期仅通过 owner-scoped 的 `/v2/kol/managed-followers/follow/{status,settings,lifecycle}` 开放；保存不启用交易。
...
已登录用户的带单访问响应始终返回其自身 KOL 资料状态；没有管理员授权时仍须显示“待管理员授权”，不得把带单入口静默隐藏或误导 KOL 使用普通跟随者设置。KOL 的托管账户按账户独立设置和启用，普通跟随者表单只对非 KOL 用户显示。
