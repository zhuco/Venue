...
终端下单队列使用通知唤醒，数据库 `0044` 在命令提交后通知单例 Executor；100ms 持久账本发现仍作为兜底。认证私有流变更将服务端事实发布提前至 33ms，保留连续性、签名基线与订单收敛检查。`POST /v2/kol/terminal/account/events` 逐次校验会话和凭证归属；无参数保留完整快照契约，`?compact=1` 首帧完整同步，此后只复用当前连接中未变化的成交及仓位历史，账户、凭证、私有代次和前帧观察时间必须匹配。其余字段始终发送当前值，不伪造观察时间。推送通知合并为最多每 500ms 读取一次最新事实，避免其他账户的更新造成旧快照积压；数据库通知本身不携带交易事实。桌面在 10 秒内未收到完整且观察时间推进的账户帧时重连并补读，零碎字节不能延长等待。账户显示延迟仅作提示，不禁用手动开平仓、撤单或改单；提交后的服务端身份、数量和签名事实校验继续执行。桌面账户切换取消旧读取，快照仍带账户选择代次，HTTP ACK 不能直接成为已挂单线。
