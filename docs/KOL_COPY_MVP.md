...
启用的 KOL 可添加已获账户所有者委托的 Binance API，填写标签、Key、Secret，并选择定比或定额；默认定比 1 倍，定额必须填写单笔报价币金额。单次对话框最多 10 个、每 KOL 最多 200 个有效账户。每个托管账户建立不可登录的内部用户，凭证、跟单授权、管理映射及该 KOL 的不可变归属在同一事务中提交。普通受邀跟单用户自行添加 API 时使用同一授权选择。

Web 不再显示页面下方独立的全局跟单表单。托管账户及已有跟单关系的本人账户从对应账户行打开设置弹窗，带单账户不展示无关系的跟单设置。跟单中和激活申请中明确显示只读原因及暂停后修改步骤。新增账户与跟单设置统一使用“定比跟单 / 定额跟单”。定比默认 1 倍，定额只填写每笔报价币名义金额（订单价值，不是保证金），不显示倍数。页面不设置总跟单金额或项目止损；资金采用账户验证取得的全部 USD 权益快照，保存设置时使用该账户的验证权益，缺失或非正权益时禁止保存并提示验证。该快照不会随盈亏自动更新。单笔/总名义上限、偏离限制和交易对保留在“高级设置”，编辑时保留已有值；运行中仍须先暂停并完成撤单对账。

凭证复用现有 AES-256-GCM、AAD 和唯一 Key 指纹。保存请求按 KOL 与 request_id 幂等，同 ID 不同内容拒绝；重复 Key 整笔回滚。页面和接口只返回管理编号、标签、掩码、验证状态、Portfolio Margin USD 权益、可用保证金及余额观察时间，不返回凭证编号、内部用户、完整账户身份或密钥。验证复用 Binance 只读 probe，并在同次签名账户读取中取得 `accountEquity` 与 `totalAvailableBalance`；拒绝跨 KOL 访问及非启用 KOL 操作。保存即形成跟单授权，但不授予带单权限、选中终端账户、创建机器人或直接发送订单。

托管账户验证成功后按已保存授权自动建立关系并申请激活，不要求第二次风险勾选。内部暂停态只承载空仓、无挂单、权限、账户归属和单 writer 的激活闸门；迁移 `0038` 保存授权。`/v2/kol/managed-followers/delete` 不再要求登录密码：一次点击即暂停关系、取消尚未发送的复制命令、从托管列表隐藏并开始撤销程序子单。已发送或结果未知的命令继续按原 `clientOrderId` 对账，相关镜像终结后由 Control 擦除 API 密文和 Key 指纹；不以删除请求自动平仓，也不删除 Binance 子账户。界面不保留已删账户条目；为防重复下单，关系、命令和成交的最小审计事实仍保留。

每账户独立选择定比或定额跟单，迁移 `0032` 的旧记录默认定比。定比保留原数量比例公式；定额使用正数报价币名义金额除以限价、止损触发价或市价源单首次成交价，不再乘资金比例或倍数。两种模式的开仓数量低于交易所最低名义额时向上补足，达到最低额后按交易所步长向下取整且不超过计划金额；边界上若向下会失去合规性，则取最小合规数量。实际金额仍受账户总风险上限和交易所最大数量限制。已有子单成交先扣减；只减仓数量仍向下裁剪，不补足开仓最低额。主账户以名义额在原开仓 99%–101% 内的反向限价单退出一个曾因最低额补足的来源仓位时，子账户平掉该实际补足数量，再按当前可减仓数量和已有平仓委托裁剪。命令持久化取整策略，旧命令回读保留原规则，详见 [人工带单数量契约](LEADER_ORDER_MIRROR.md#同步规则)。改变运行中参数仍须先完成撤单和对账。
...
4. 用户明确确认风险参数并启用后，同步已授权人工带单主账户的新限价单、市价单和 `STOP_MARKET` 止损单；子账户独立成交。
...
- 止盈、追踪止损、close-all 和其他 Algo 类型；本阶段复制普通 UM 限价单、经认证身份确认的市价单及 `STOP_MARKET` 止损单。
...
普通 Windows 终端免费开放：使用 `POST /v2/account/terminal/register` 注册，不要求邀请码、不创建 KOL 角色或跟单关系；登录后可绑定验证自己的 API、查询账户及管理 Binance Grid。Web 登录页提供“注册跟单账户”入口，`/register` 要求填写 KOL 邀请码；`/join/<invite_code>` 自动带入邀请码。注册后进入“我的跟单账户”，只通过带跟单授权的接口添加本人账户，并选择定比或定额；不会获得 KOL、托管他人账户或带单权限。Web 注册专用于跟随者，使用 `POST /v2/account/register`，必须提供有效邀请码，并在同一事务中绑定 KOL。两端共享用户登录和凭证服务，但注册入口不可混用；客户端不得通过注册字段自行指定 KOL 权限。付费 KOL 由管理员开通，自动收费与结算暂不实现。
...
- KOL 后台通过 `GET/POST /v2/kol/invite` 查看、随机生成或自定义邀请码，公开链接为 `/join/<invite_code>`。自动生成默认 6 位，自定义为 4–64 位 ASCII 字母和数字，去除两端空白、区分大小写；全局 `code_hash` 唯一约束覆盖所有 KOL 及已停用历史码，不允许另一个 KOL 复用。每个 KOL 只有一个当前有效码，更换前须确认旧链接失效；冲突与失败在事务内回滚，旧码保持有效。
- 邀请码是归属标识，不是登录凭证或交易授权。服务端使用哈希匹配；迁移 `0046` 为后台新建码保存 AES-256-GCM 密文，AAD 绑定 KOL 与邀请 ID，仅所属 KOL 可查看原文。旧记录仅有哈希时仍可注册，但不能还原展示，须显式更换。请求以 UUID 和完整请求哈希幂等，期望当前邀请 ID 防止并发覆盖。普通跟随者无生成权限。
...
- 已签名确认的普通/止损子单不因跟随私流投影暂未包含它而撤销或重建；源单变化及停止才触发撤单，原身份和精确回读边界见 [人工带单](LEADER_ORDER_MIRROR.md)。
...
限价委托对账按原 `clientOrderId` 签名精确查单并核对原生身份、方向、数量和价格，不先补读账户成交历史。常规发送前的当前账户快照不复用前一命令的成交游标；市价结算仍保留本命令的持久基线。账户待对账时独立精确撤单仍按账户串行领取；正在发送的命令、同目标未决撤单、撤后重挂及所有新挂单继续受栅栏约束。撤单自身超时后也必须按原身份独立对账。
...
完整授权、生命周期、数量、改单/撤单、恢复及 Web 边界见 [人工带单与订单同步](LEADER_ORDER_MIRROR.md)。
...
主账户启用后的普通 UM GTC/PostOnly 限价单、经认证成交或签名精确查单确认的市价源单及 `STOP_MARKET` 止损单，按各跟随账户的定比或定额设置同步。市价源单按原生订单号只复制一次；止损单修改时先精确撤销旧 Algo 子单再创建替代单，主单撤销时同步撤销。允许交易对留空表示允许 Binance 当前在交易的 USDT/USDC 永续合约。单关系最多保留 1000 个活动订单映射，交易所及账户自身限制仍独立生效。不补追限价单成交或主从仓位差异，也不复制其他 Algo 类型。

限价主单已确认全部成交后，已有未成交子单继续按原价等待，不因源单退出交易所活动挂单列表而撤销；全成事实保存在原镜像中，不从历史成交补建或复活已结束的子单。主单撤销、改单及暂停/撤权仍遵循撤销剩余子单契约。
...
5. Portfolio Margin 权益、可用保证金、持仓、普通开放订单和 Algo 开放订单签名面均完整可读；任一面缺失、解析失败或网络失败都不能显示验证通过。
...
实现入口与管理员授权操作见 [人工带单与订单同步](LEADER_ORDER_MIRROR.md)。源码、离线 fixture、隔离 PostgreSQL、浏览器、目标主机容量及真实 Canary 分别验收；前者通过不等于后者完成。真实部署、旧账户停止或迁入、真实交易必须另获当前任务明确授权，不沿用历史计划中的金额或停机许可。
...
Binance 公共 WebSocket 优先通过本机已配置的网络直连交易所，连接失败回退原中继。REST 历史补页保留原路径；行情线程直接请求绘制，盘口、成交和 K 线默认显示取样为 33ms。Bybit 当前图表使用公开 WS 的深度 50 序列盘口、逐笔成交与所选周期 K 线，展示前截取前 20 档；历史补页及两秒一次的全市场统计独立运行，断线回退 REST 并重新取得 WS 快照。其余四所当前公共展示仍为 REST 快照，不据此宣称达到 300ms。

终端下单队列使用通知唤醒，数据库 `0044` 在命令提交后通知单例 Executor；100ms 持久账本发现仍作为兜底。认证私有流变更将服务端事实发布提前至 33ms，保留连续性、签名基线与订单收敛检查。`POST /v2/kol/terminal/account/events` 逐次校验会话和凭证归属；无参数保留完整快照契约，`?compact=1` 首帧完整同步，此后仅发送变化的仓位、挂单、条件单、成交、仓位历史和资金字段；字段缺省表示保留，空数组表示清空，账户、凭证、私有代次和前帧观察时间必须匹配。每次更新携带真实的观察时间，不伪造心跳时间；账户或私有代次变化时重新发送完整快照。桌面成交及仓位历史各限最近 100 条，每 5 秒补读一次，同连接其余更新不查询历史表；不改变执行对账读取范围或删除已持久化记录。推送通知合并为最多每 500ms 读取一次最新事实，避免其他账户的更新造成旧快照积压；数据库通知本身不携带交易事实。桌面在 10 秒内未收到完整且观察时间推进的账户帧时重连并补读，零碎字节不能延长等待。账户显示延迟仅作提示，不禁用手动开平仓、撤单或改单；平仓、撤单和改单准入仅读取当前订单/仓位状态，不查询展示历史；提交后的服务端身份、数量和签名事实校验继续执行。桌面账户切换取消旧读取，快照仍带账户选择代次，HTTP ACK 不能直接成为已挂单线。

原生桌面 `Ctrl+Shift+L` 提供默认关闭的 120 秒有界延迟采集及 JSON 导出，帧缓冲回执为含读回开销的渲染上界，不代表显示器扫描输出或 LIVE 验收；时钟、样本及缺口见 [VenueFlow 延迟证据](VENUEFLOW_LATENCY_EVIDENCE.md)。

目标为正常连接下行情事件到可见绘制 P95 ≤300ms、点击下单到已确认挂单线 P95 ≤1000ms。验收必须校正时钟误差，记录网络接收、提交、交易所确认、账户快照接收和实际绘制时间；只读行情测量与离线队列测试不能替代真实下单端到端验收。

Web 的“我的同步订单”只面向跟随用户，以 `follower_user_id` 查询本人订单。KOL 后台不展示此模块，不能将 KOL 作为跟随者的空结果解释为其名下跟单账户没有订单。

KOL 与跟单页面必须使用币安统一账户（Portfolio Margin）及 U 本位双向持仓。`/v2/kol/source` 保存 KOL 唯一带单账户，不使用会话当前账户代替。管理员可预建无账户的 draft KOL；首次选择本人已验证账户时以实际权益（允许为零）初始化策略资金并占用全站 5 个 KOL 名额之一，不自动启动带单。已有机器人或跟单关系时禁止换源，保留历史归属。需迁移 0047。
...
指定带单账户只验证账户归属和币安账户/API 模式，不要求提前入金。零权益可保存绑定，但不会自动创建或启动机器人；机器人配置的正策略资金校验保持独立。页面显示保存成功或明确的服务端拒绝原因。
...
共享 Executor 的库存做市新增迁移 `0049` 观测摘要：相同健康私流代次、原 5 秒时效及账户串行门保持不变，仅时间更新不再废弃未变化的账户事实；旧未决命令按原精确观测规则对账。
