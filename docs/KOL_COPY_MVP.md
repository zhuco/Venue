...
- `apps/venue-web`：现有响应式设计、同源 BFF、安全头、Decimal 展示和实时状态组件；必须替换受控环境会话，而不是另造第二个 Web。
...
- 在现有 `apps/venue-web` 增加 `/join/<invite_code>`、注册/登录、API 管理、跟单状态、KOL 页面编辑和 `/terminal`；不新建第二个前端。

这样只新增一个 binary 和一组局部模块，旧 Grid/六所 Node 不迁移、不改数据格式，也不成为交付依赖。

已实现的 P1 边界是 `accounts/kol.rs` 的邀请码哈希解析、注册事务归属和 KOL 本人 revision 更新，`http/accounts.rs` 的公开/认证路由，以及 `venue-web` 的 `/join/<invite_code>`、登录注册 Cookie BFF 与 `/kol/profile`。P2 已把加密凭证的保存、验证、选择、删除以及风险参数、暂停和显式启用申请接入固定 BFF 路由；`0018_kol_follow_activation.sql` 使启用请求保持 Pending，只有后续 Executor 完成签名 KOL/跟随账户基线才能改为 Active。BFF 只把 Control token 封在签名 `HttpOnly` Cookie 中，写操作仍要求同源和 CSRF；公开页面只渲染纯文本资料和固定风险提示。这些入口不包含 `venue-executor-binance` 物理循环，不能据此启动实盘。
...
当前已实现：BFF 只允许固定凭证、风险参数和生命周期路径；服务端从不可变邀请绑定及凭证归属导出 KOL 和账户，且仅接纳已验证、最近验证新鲜、无敞口的凭证。显式确认只写 Pending 基线请求；尚未存在的 Executor 不会把它改为 Active，也不会发送订单。
