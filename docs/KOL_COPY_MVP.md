...
终端下单队列使用通知唤醒，数据库 `0044` 在命令提交后通知单例 Executor；100ms 持久账本发现仍作为兜底。认证私有流变更将快照发布提前至 33ms，保留连续性、签名基线与订单收敛检查。`POST /v2/kol/terminal/account/events` 返回经过会话和凭证归属校验的完整账户快照，数据库通知只触发读取，不携带交易事实；推送断线后重新订阅并补读。桌面账户切换取消旧读取，快照仍带账户选择代次，HTTP ACK 不能直接成为已挂单线。
