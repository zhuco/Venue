...
## v0.1.0-alpha.20 — 2026-09-04

- 策略启动先建立认证用户流，签名 REST 仅建立基线和恢复异常缺口；正常订单、双向仓位及连接覆盖时间由连续私流维护，不定时拉全账户快照。
- Grid 普通成交滚动使用成交参考价，不再依赖 BBO；盈利减仓候选独立签名核验 PM 权益，失败只推迟减仓。余额保留原始观察时间，不以心跳伪造新权益。
- 读模型等待匹配的订单/仓位事件与命令结算，重复成交仍由 PostgreSQL 去重；空闲不是断流。10 ms 仍为出网验收目标，不能以离线测试代替实盘测量。

## v0.1.0-alpha.19 — 2026-09-04

- REST 与认证私流成交去重按精确 Decimal 数值比较价格/数量，兼容历史行的小数尾零差异；其余成交身份和代际栅栏保持严格，不覆盖已分配成交。
- 成交冲突仅输出不一致字段名，行情失败区分 BBO/规则与 USD 汇率证据，避免将所有补撤停顿归为连接异常。
