...
- 桌面增加同用户单实例锁：重复启动退出，进程结束自动释放；此补充修复以独立提交发布。

- 整合桌面账户切换、订单与成交标记、批量撤单、平仓预留诊断、实时指标与 K 线加载恢复。
- 新增桌面分段延迟采集与导出，保留真实行情及签名订单可见性的独立验收边界。
- 整合 Bitget 带单凭证准入、马丁支撑回调与整点失效判断、最小数量取整及启动诊断。
- 整合库存做市私流驱动、临时读取失败恢复及网格平仓层数恢复，保留原订单身份待对账。
- 汇入全部本地与已获取远程分支，保留原提交历史；版本记录不代表部署或实盘验收完成。

## v0.1.1 — 2026-09-08

- 统一桌面字体、交易对标签、独立价格轴和实时指标读数，精简工具栏与交易确认弹窗，保留仓位历史。
- 整合 KOL 邀请、唯一带单账户、账户验证反馈、注册与图文使用指南。
- 整合库存做市稳定性、签名事实刷新、跟单数量及终态对账修复。
- 合并 Mac 本地部署与打包贡献及用户教程，保留贡献者提交历史。

## v0.1.0 — 2026-09-07

- 新增 Binance 独立库存做市：每实例双边 Post Only 报价、净库存偏移、波动半价差、单腿/总量/净头寸和账户权益保护；复用单例 Executor、账户串行队列与原身份待对账边界。
- 策略分配改由用户管理，不以同账户已有 Grid/KOL/做市实例阻止启动；旧 Node 与新 Executor 的真实账户互斥仍保留。
- 图表拖动普通限价单改为确认后的“撤单并重新挂单”：旧单按原身份完成签名终态回读后，才按剩余数量释放新单；新单保留原 GTC/Post Only、方向和开平仓语义。
- 修复终端历史将其他交易所独立策略命令混入 Binance 查询而导致 503 的问题；桌面在历史刷新失败时明确标记旧的非终态回执，避免将“等待执行”误作当前状态。
- KOL 跟随命令继续以实际子单成交和签名持仓核对为准，撤单/新挂与不确定结果均不自动重发。

## v0.1.0-alpha.31 — 2026-09-07

- 最低额补足平仓的来源匹配改以限价名义额而非数量判断，覆盖同一固定报价币金额在不同价格下产生不同 DOGE 数量的正常情形；99%–101% 的反向来源限价单会平掉子账户实际补足数量。

## v0.1.0-alpha.30 — 2026-09-07

- 对曾因交易所最低名义额向上补足的限价开仓，带单账户以不少于原开仓 99% 的反向限价单退出时，子账户以实际建立的补足数量平仓；仍受新鲜持仓、已有平仓预留和数量步长约束。部分平仓继续按跟单比例或固定名义金额处理。

## v0.1.0-alpha.29 — 2026-09-07

- 终端手动 Post Only 开仓合并账户绑定、验证状态和凭证读取，不再为开仓读取完整账户/持仓快照；交易对规则继续复用共享缓存。
- 市价平仓/反开改为执行专用投影读取，合并凭证归属、账户和私流健康检查，避免加载成交及仓位历史；保留新鲜持仓数量裁剪、命令幂等和超时同 ID 对账。
- 发送前拒绝按绑定、数量、价格、方向、订单类型、规则目录、签名、请求准备和账本冲突分别入账及显示。旧通用错误码不再猜测失败原因或误称为交易所拒单。

## v0.1.0-alpha.28 — 2026-09-06

- 合并五所独立策略、网格与耐久命令执行，保留单例 Executor、账户串行和未知结果原身份对账。
- 补齐 Bybit、Bitget、Gate、OKX、Hyperliquid 签名成交回读、条件单与市场订单兼容修复；Binance 目录跳过不可规范化交易对。
- 增加支撑分批做多策略、迁移 0035–0037、用户 API 和桌面配置、签名预检及生命周期界面。
- 整合 KOL 托管绑定来源兼容、开仓数量向上合规取整与真实数量结算，更新桌面布局、HTTPS 路由和 Web 跟单设置。
- 本条记录源码范围；构建、服务器运行及逐所实盘验收分别核验，不以版本号替代验收。

## v0.1.0-alpha.27 — 2026-09-05

- VenueFlow 将内置 Grid 与 KOL 人工带单统一为私有交易机器人列表，可新建、编辑、启用、停用及重试停用，不包含策略广场。
- 同一真实 KOL 主账户最多保存 20 份带单机器人配置；同一 KOL 同时只允许一个非停止实例，防止重复跟单。停用继续撤销程序子单但不主动平仓。
- Control 增加多机器人目录、配置修订和生命周期接口，旧单实例接口保持兼容；Executor 只加载当前运行实例，并沿用账户串行、稳定命令身份与待对账边界。

## v0.1.0-alpha.26 — 2026-09-04

- 同一认证账户的非本 Grid/KOL 交易对私流事件不再丢弃连续投影、触发签名全账户纠偏或延后本交易对滚动；混合 `ACCOUNT_UPDATE` 只折叠已订阅交易对。缺字段、同范围订单身份冲突、乱序和未知账户事件仍失败关闭并保留签名纠偏。

## v0.1.0-alpha.25 — 2026-09-04

- 将 PostgreSQL Grid revision CAS 冲突明确分类为“冷收敛已被认证成交批次抢先提交”：冷路径重新读取事实而不报存储故障、阻塞或改变生命周期。真实数据库不可用仍保留错误日志与失败关闭语义；热路径冲突仍请求签名恢复。

## v0.1.0-alpha.24 — 2026-09-04

- 已验证、无未决命令的周期性私流订单面直接持久化并被网关接受，不再每 500 ms 反向取消 Grid 冷收敛事务；认证成交与失效信号仍立即打断冷路径。修复 SQLx/PostgreSQL 的伪事务告警根因，不通过压制日志或重置网格掩盖。

## v0.1.0-alpha.23 — 2026-09-04

- 实际下单连接启动校时后每小时后台刷新一次，忙连接或失败按一分钟重试；刷新期间保留已验证偏差，成功原子替换，不持有账户下单锁等待网络。
- 连续私流用规范成交增量核对实际双腿仓位，不要求账户更新与成交消息时间戳精确相等；更新未覆盖成交、数量冲突、重复成交内容冲突仍拒绝发布并保留恢复语义。

## v0.1.0-alpha.22 — 2026-09-04

- 策略私流基线启用前预热实际共享下单连接的 Binance 签名时钟；正常成交不再因进程重启后连接未校时而退回完整 REST 预检。失败保留未就绪状态，不跳过校时或放宽签名要求。
- 成功使用认证热事实派发时输出明确路径日志；账户和交易对连接仍独立，启动准备与实际发送复用同一个路由器实例。

## v0.1.0-alpha.21 — 2026-09-04

- Grid 热路径允许未获得交易所订单号、零成交且已终态的历史命令共存；当前挂单、已成交记录及实际成交匹配仍严格验证原生身份，不删除或改写历史记录。
- 已绑定记录版本、前驱批次、私流代际和成交顺序的跨微批连续规划，不再被上一批的待收敛标记误拦截；无有效连续缓存的脏状态仍拒绝。
- 热路径拒绝日志保留具体叠加校验类别，分别记录成交至规划唤醒、首末发送入口，10 ms 目标继续以真实成交测量验收。
