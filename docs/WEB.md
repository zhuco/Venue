...
Overview, relations, accounts, orders, positions, fills, reconciliation, ledger/drift and semantic controls exist. They are an operator console, not the required public product. The current environment-injected BFF session must be replaced with per-user Secure/HttpOnly sessions; existing account ownership checks remain reusable.
...
The following values describe the currently submitted operator console and remain only until MVP P1 replaces its bootstrap session. Set them through the deployment's process environment, never in browser storage or Git:
