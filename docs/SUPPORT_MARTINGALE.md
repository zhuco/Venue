...
逐币可选 `stop_loss`：`fixed_price` 使用固定价格；`average_price_percent` 使用签名持仓均价乘以 `(1-rate)`。缺失参数保持旧配置的“未启用止损”。migration 0039 扩展原命令关联类型以持久化止损及止盈撤销；这属于当前源码扩展，不代表已部署或完成实盘验收。桌面从“新建机器人 → 马丁做多”进入配置，主界面只保留统一列表，详情及管理按需打开。

止损由共享 Executor 根据不超过 5 秒且非未来的签名标记价触发，先撤本实例止盈并等待精确终态，再按当前同代签名持仓裁剪只减仓市价单。第一次入账即暂停整个实例增险；撤单后价格反弹不解除止损。发送不确定时沿用原命令/客户单号对账，明确拒绝、撤销或零成交失败进入逐币 `sl_failed` 和需要关注，不自动重发。该机制依赖服务及行情可用，不是交易所常驻止损单，不保证指定价格或亏损上限。
...
|---|---|
| 方向 | 只做多；不建立空头或对冲腿 |
| 首仓 | 支撑位模式使用环境及反弹确认；固定价格模式到达逐币阈值后市价买入 |
| 补仓 | 支撑位模式要求更低新支撑和反弹确认；固定价格模式按逐币跌幅下移阈值，均使用市价单 |
| 止盈 | 按执行所实际成本计算的普通 GTC 只减仓限价单；不采用触发后市价的 TP Algo |
| 持仓时间 | 不设自动超时平仓；信号有效期和订单超时仍须有界 |
| 止损 | 默认关闭；逐币可选固定价格或均价下跌百分比，服务端触发市价只减仓并暂停增险 |
...
| 首仓/补仓 | `first_order_notional=5` 是目标执行报价资产名义金额；若交易所最低名义额或数量步长要求更高，按规则向上取整并以实际合约名义额计入预算；`max_entries=10` 含首仓；研究默认 `size_multiplier=1.25`，同时测试 1.0 对照 |
...
目标金额不满足某交易所最低数量/名义金额时，按该所数量步长向上取整；取整后的实际合约名义额必须同时满足最小量、最小名义额、最大量和实例总预算，否则明确显示“金额不足”并跳过。实际名义额写入计划、持久命令和预算预留，不能使用未取整的目标金额。候选币不足不凑满 30，资金不足不凑满 10 个持仓。
...
规划先对所有满足价格、未消费及更低补仓约束的支撑检查新鲜回调，再从已确认回调中选择最近确认的支撑；不能先选最新支撑而遮蔽其他区域的信号。回调触及线必须在支撑确认后开盘。支撑周期收盘价使用同一连续参考序列中该周期最后一根15m线的收盘价，周期内部的短暂收盘下穿不单独判定1h支撑失效。
...
首仓和补仓使用领域 `ExecutionCommand::PlaceMarket`，按执行所数量步长向上补足最低开仓名义额，并以实际合约名义额持久化；实例预算在命令入账事务内按取整后名义额预留；发送前仍按新鲜事实重做精度、数量、最低名义额、最大量与账户交易额度检查。持久身份至少绑定账户、实例、run、cycle、symbol、support、动作与决策序号；哈希/编码由既有身份边界完成，不让 adapter 随机改 client ID。
...
配置模态绑定打开时的 credential/account、校验 revision、保护 pending 请求并固定底部按钮。保存失败保留草稿，成功只建立 Stopped 实例。启动前显示名义预算、首仓、层数、补仓倍率、仍会在下跌补仓、不自动止损和不限持仓的摘要；账户或凭证切换后原草稿不能提交。
...
桌面只有当前 revision 的预检通过后才开放启动确认；Control 处理 Start 时仍重新执行同一签名预检并在账户事务内复核排他。CLI 只保留暂停/Draining 等减险生命周期操作，Start/Resume 必须走用户作用域 Control 准入。控制请求超时只表示实例状态待刷新核对，不使用订单的 `ReconcileRequired` 术语，也不自动重发；物理订单不确定仍严格进入原命令 ID 的 `ReconcileRequired`。
...
缺少执行所深度、费用或资金费历史时，允许完成规则研究，但该所执行回测标记未完成。真实带单另验证跟单者漏掉低位补仓、资金不足和成交偏差；本策略开发不改变 KOL 市价源单按原生订单号只复制一次的契约，也不自动开通平台带单。
