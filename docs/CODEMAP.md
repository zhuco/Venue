...
| 远程 Control 桌面启动 | `apps/ui/desktop/src/server_connection.rs`、`scripts/Start-VenueFlow.ps1`、`scripts/configure_desktop_https.py` | 桌面默认直连 `https://clawdbotweb.site`；设置页校验地址、暂存编辑并显式应用重连，schema 7 一次性迁移旧开发默认地址而不迁移凭据。Caddy 精确转发桌面接口/SSE 到本机 Control；既有路由恢复服务用 `venue-desktop-https.conf` 在启动后重建该路由。启动脚本仅显式 `-SshTunnel` 时使用 SSH |
