...
带单机器人与挂单同步入口：`apps/venue-control/src/accounts/leader_bot.rs` 管理本人最多 20 条私有配置及单活动实例生命周期，`leader_bot_admin.rs` 和 `bin/venue-leader-bot-admin.rs` 管理默认拒绝的用户授权；`order_mirror/{planner,store,settlement}.rs` 保存源/子单映射、先撤后挂和精确对账。协议在 `crates/venue-control-protocol/src/leader_bot.rs`，迁移为 `0028`/`0029`/`0034`，版本化安装在 `schema.rs`。桌面 `leader_bot_view.rs` 与 `grid_view.rs` 组成统一内置机器人列表及类型专属编辑/生命周期面板，`ui/status_bar.rs` 区分连接失败与事实过期；本阶段无策略广场。用户 Web `/`、`/join/[invite_code]` 与 `lib/customer-server.ts` 继续使用兼容单实例接口；运营控制台位于 `/ops`。契约见 [LEADER_ORDER_MIRROR](LEADER_ORDER_MIRROR.md)，实库回归见 `tests/support/leader_order_mirror.rs`。
...
| VenueFlow 桌面终端 | `apps/ui/desktop/src/main.rs` | `account_client.rs` 的免费注册调用 `/v2/account/terminal/register`，Web 仍用必填邀请码的 `/v2/account/register`。`trading.rs`、`trade_dock.rs` 统一按钮、设置和非重复热键；默认 `A/S/D/F` 开多、平多、开空、平空，Maker-only 默认开启。`workspace.rs` 提供图表/盘口/下单/账户区。`execution_view.rs` 和 `client/execution.rs` 的仓位、挂单、成交、仓位变更历史及资产只读新私有投影，命令历史只读新账本；空响应与切换凭证清除旧数据，过期/失败明确提示，不回退冻结 Node 数据。`grid_view.rs` 与 `client/grid.rs` 是唯一桌面 Grid 配置及生命周期入口，不重复展示旧策略。Post Only 四动作、确认后市价平仓及精确撤单进入唯一 Executor。|
| 桌面账户与下单反馈 | `apps/ui/desktop/src/{account_center,terminal_feedback,client/terminal}.rs` | API 管理为多行表格；上下账户备注读取同一选中凭证，连接状态读取同一私有投影。有效登录会话、API 验证及会话选择可在重启后恢复。下单面板和历史委托展示安全失败原因/请求号，超时不自动重发，账本确认不冒充成交。 |
