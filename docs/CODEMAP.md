...
当前可复用入口：`apps/venue-control/src/accounts/`（注册、登录、会话、凭证密文、邀请码归属、KOL 与跟单设置）、`crates/venue-gateway-binance/src/credential_probe.rs`（Portfolio Margin/UM/双向持仓只读验证）及 `apps/venue-web`（Next.js/BFF）。KOL MVP 已落地 `0017_kol_copy_mvp.sql`、`0018_kol_follow_activation.sql`、`venue-control-protocol/src/kol.rs`、单 Executor PostgreSQL 锁边界、P1 用户入口以及 P2 凭证/风险参数 BFF；启用仅创建待 Executor 基线核对请求，物理 Executor 循环仍未实现，开发顺序见 MVP 契约。
...
旧 Web 控制台仍使用 BFF 环境注入的运维会话；KOL 用户入口另使用真实 Control 会话封入签名 `HttpOnly/Secure/SameSite=Lax` Cookie。HTTP/SSE 继续保留 Control 账户归属校验，不复用 Node token；部署配置见 `docs/WEB.md`。
...
| 冻结旧签名快照与成交恢复 | `crates/venue-execution/src/account_snapshot.rs`、`account_cursor_tests.rs` | 余额保留原资产、未知可用金额不填零；订单状态与已成交量只保留来源明确的值。`AccountRecoveryRequest::previous_fills_cursor` 从当前 checkpoint 恢复，整轮签名快照成功才原子推进游标。HL 的账户级协议在 `crates/venue-gateway-hyperliquid/src/protocol/account.rs`，覆盖非所选币种的仓位/订单/成交、Net 正负数量与保留窗口缺口；单交易对旧 parser 不可冒充账户完整性 |
...
| Binance Portfolio Margin adapter | `crates/venue-gateway-binance/src/account_gateway.rs` | `credential_probe.rs` 已提供账户/API、Portfolio Margin UM、交易权限与双向持仓只读验证；`execution.rs` 已补 Hedge Mode 显式 `PlaceMarket` 预备请求并禁止原生 `reduceOnly`，`transport.rs`、`private_ws.rs`、`readback.rs` 仍由后续轻量 Executor 复用，不复制签名或原生 DTO。旧 `recovery.rs` 不进入新链 |
...
| 版本化 Control 协议 | `crates/venue-control-protocol/src/{lib,accounts,kol}.rs` | 既有 schema v2 Node/Control DTO 保持冻结；KOL schema v1 另固定邀请注册、公开页、跟单风险/启停、基础终端和 `Pending/Sending/Accepted/Rejected/ReconcileRequired/Reconciled/Cancelled` 状态，不复用旧 delivery/Actor 协议，也不授予物理交易权限 |
| Control 服务核心 | `apps/venue-control/src/{lib,accounts/kol,kol_executor,executor_store,executor_secret,executor_exchange,http/accounts}.rs` | `0017_kol_copy_mvp.sql` 提供最多5个启用KOL、200个启用关系、不可换绑归属、跨用户凭证/账户外键、源成交/目标及统一 Binance 命令账本；`0018_kol_follow_activation.sql` 将显式启用保留为 Pending 基线请求。`executor_store.rs` 持久化源成交去重、非终态恢复查询和通过 revision 的激活提升；`executor_secret.rs` 仅按 credential+owner 读取并解密密文；`executor_exchange.rs` 是离线 mock 的 clientOrderId/回读边界。`kol_executor.rs` 原子领取命令、同账户 `Sending/ReconcileRequired` 栅栏、私流成交映射和有界中心调度。`kol_mvp.rs` 提供独占数据库会话持有的全局 advisory lock。`venue-executor-binance` 仅组装这些受限依赖，事件循环仍属 P3；旧 `venue-copy-worker` 不进入 MVP 发布链 |
...
| 响应式用户 Web | `apps/venue-web/{app/join/[inviteCode],app/kol/profile,app/follow/setup,app/api/kol,components/kol-*}.tsx` | Next.js 16 + React 19 + TypeScript 的同源 BFF。公开 join 页只读解析 KOL；注册/登录由 BFF 写入签名的 `HttpOnly/Secure/SameSite=Lax` 用户 Cookie，Control token 不进入前端响应或存储。`/follow/setup` 只转发保存、验证、选择、删除凭证及受限风险/生命周期请求，写操作要求同源和 BFF CSRF；申请启用不会直接变 Active。旧 `control-console.tsx` 是冻结运维界面，不是 MVP 用户入口；页面已建立不等于执行或实盘验收完成 |
...
| KOL 离线容量与发布演练 | `apps/venue-control/tests/kol_executor_capacity.rs`、`scripts/Invoke-KolCanaryDrill.ps1` | 确定性 5 KOL / 200 follower 中心调度 fixture 验证有界队列、串行账户与 fan-out 启动窗口；脚本必须显式 `-OfflineFixture`，拒绝进程内 Binance 凭证并只调用受控 Rust 构建入口。它不是 2核4G 实机容量证明或真实 Canary |
...
- VenueFlow 图表实时锚点：`apps/venueflow/src/chart.rs` 与 `chart_view.rs`；左键拖动保持固定 K 线宽度、右侧留白占用视窗槽位，新 K 线在拖定位置继续更新。内联测试覆盖连续拖动/松手不回弹、图表与时间轴、缩放及新 K 线到达；“跟随”/“适配”显式恢复右端位置。
