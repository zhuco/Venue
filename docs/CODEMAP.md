...
当前可复用入口：`apps/venue-control/src/accounts/`（注册、登录、会话、凭证密文、邀请码归属、KOL 与跟单设置）、`crates/venue-gateway-binance/src/credential_probe.rs`（Portfolio Margin/UM/双向持仓只读验证）及 `apps/venue-web`（Next.js/BFF）。KOL MVP 已落地 `0017_kol_copy_mvp.sql`、`0018_kol_follow_activation.sql`、`venue-control-protocol/src/kol.rs`、单 Executor PostgreSQL 锁边界、P1 用户入口以及 P2 凭证/风险参数 BFF；离线运行时只收敛账本，真实私流/签名发送仍未接通，不能用于实盘。
...
| Control 服务核心 | `apps/venue-control/src/{lib,accounts/kol,kol_executor,executor_store,executor_secret,executor_exchange,executor_runtime,http/accounts}.rs` | `0017_kol_copy_mvp.sql` 提供最多5个启用KOL、200个启用关系、不可换绑归属、跨用户凭证/账户外键、源成交/目标及统一 Binance 命令账本；`record_source_fill_and_plan` 在同一事务去重成交、推进每关系/腿目标并生成确定性 `clientOrderId`。`executor_runtime.rs` 对 Pending 只 POST 一次、对重启后的 Sending/Accepted/ReconcileRequired 只按同一 ID readback，任何不确定结果保持 `ReconcileRequired`。`executor_secret.rs` 仅按 credential+owner 解密；`executor_exchange.rs` 是离线 mock 边界。`venue-executor-binance` 取得单例锁后只允许显式离线 fixture sweep；未接入私流、真实规则/仓位裁剪和签名 adapter 前拒绝实盘启动。旧 `venue-copy-worker` 不进入 MVP 发布链 |
