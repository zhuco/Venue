...
当前可复用入口：`apps/venue-control/src/accounts/`（注册、登录、会话、凭证密文）、`crates/venue-gateway-binance/src/credential_probe.rs`（Portfolio Margin/UM/双向持仓只读验证）及 `apps/ui/web`（Next.js/BFF）。两套 UI 的入口判断先看 `apps/ui/README.md`。KOL MVP 已落地 `0017_kol_copy_mvp.sql`、`venue-control-protocol/src/kol.rs` 和单 Executor PostgreSQL 锁边界；邀请/关系 HTTP、页面及物理 Executor 循环仍未实现，开发顺序见 MVP 契约。
...
| Binance Portfolio Margin adapter | `crates/venue-gateway-binance/src/account_gateway.rs` | `credential_probe.rs` 已提供账户/API、Portfolio Margin UM、交易权限与双向持仓只读验证；`transport.rs`、`private_ws.rs`、`readback.rs`、`execution.rs` 的协议实现由轻量 Executor 复用，不复制签名或原生 DTO。旧 `recovery.rs` 不进入新链 |
...
| Control 服务核心 | `apps/venue-control/src/{lib,accounts/kol,kol_executor,executor_store,executor_secret,executor_exchange,executor_runtime,http/accounts}.rs` | `0017_kol_copy_mvp.sql` 提供最多5个启用KOL、200个启用关系、不可换绑归属、跨用户凭证/账户外键、源成交/目标及统一 Binance 命令账本；`0018_kol_follow_activation.sql` 将显式启用保留为 Pending 基线请求。`executor_store.rs` 在同一事务中去重源成交、推进关系目标并创建稳定命令 ID；`executor_secret.rs` 仅按 credential+owner 读取并解密密文；`executor_exchange.rs` 是 mock 与生产适配所共用的 clientOrderId/回读边界。`executor_runtime.rs` 重启时先回读、超时只入 `ReconcileRequired`、每账户只领取最旧 Pending。`kol_executor.rs` 原子领取命令、同账户 `Sending/ReconcileRequired` 栅栏、私流成交映射和有界中心调度。`kol_mvp.rs` 提供独占数据库会话持有的全局 advisory lock。`venue-executor-binance` 只可作为新链唯一入口；旧 `venue-copy-worker` 不进入 KOL 发布包。|
