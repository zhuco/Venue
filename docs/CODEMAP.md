...
当前可复用入口：`apps/venue-control/src/accounts/`（注册、登录、会话、凭证密文、邀请码归属、KOL 与跟单设置）、`crates/venue-gateway-binance/src/credential_probe.rs`（Portfolio Margin/UM/双向持仓只读验证）及 `apps/ui/web`（Next.js/BFF）。两套 UI 的入口判断先看 `apps/ui/README.md`。KOL MVP 已落地 `0017_kol_copy_mvp.sql`、`0018_kol_follow_activation.sql`、`venue-control-protocol/src/kol.rs`、单 Executor PostgreSQL 锁、P1/P2 用户入口和生产 `venue-executor-binance` 组装；启动仅在 explicit LIVE、PostgreSQL 和既有主密钥均有效时进行，具体边界见 MVP 契约。
...
| Binance Portfolio Margin adapter | `crates/venue-gateway-binance/src/{account_gateway,transport,private_ws,readback}.rs` | `credential_probe.rs` 提供账户/API、Portfolio Margin UM、交易权限与双向持仓只读验证；认证私流创建/续期/关闭 listenKey、有限退避与规范 TRADE 映射由 `account_gateway.rs` 统一拥有。断线、过期和语义 gap 只触发签名 REST 补读；`execution.rs` 的 Hedge `PlaceMarket` 不序列化原生 `reduceOnly`。不复制签名或原生 DTO，旧 `recovery.rs` 不进入新链 |
...
| Control 服务核心 | `apps/venue-control/src/{lib,accounts/kol,kol_executor,kol_private_source,executor_config,executor_store,executor_secret,executor_exchange,executor_runtime,bin/venue-executor-binance,http/accounts}.rs` | `0017/0018` 提供容量、归属、Pending activation、源成交/目标及命令账本。`executor_store.rs` 以 native trade identity 去重、生成稳定命令；`executor_secret.rs` 只按 credential+owner 解密；`executor_exchange.rs` 复用生产签名 transport 且以同 clientOrderId 读回；`kol_private_source.rs` 只转发规范成交与补读事实。binary 取得 advisory lock，先 activation baseline/recovery，再启动有界私流与执行循环；只接受 explicit LIVE、受限数据库 URL 和既有主密钥。旧 `venue-copy-worker` 不进入 KOL 发布包。|
...
| VenueFlow 桌面运维客户端 | `apps/ui/desktop/src/main.rs` | `trading.rs`、`trade_dock.rs` 统一按钮、交易设置与非重复热键为同一 `TradingAction`；仓位动作固定按“开多、平多、开空、平空”显示，默认应用内热键为 `A/S/D/F`，Maker-only 默认开启且用户可修改。`workspace.rs` 固定币安式图表/盘口/下单/底部账户区，`ui.rs` 的交易对标签可关闭并通过 `symbol_picker.rs` 新增；`execution_view.rs` 提供仓位、当前委托、历史委托、历史成交、仓位历史、交易机器人、资产七个标签，当前签名快照不得冒充完整委托或仓位历史，缺少服务端历史投影时明确显示不可用。顶部全局作用域栏提供行情服务器和执行账户入口；原生端以 `market_client.rs` 提供 Binance LIVE 公共行情。私有仓位、委托、成交和资产必须由 Binance gateway 在服务端解析并经唯一 Executor/Control 投影，桌面端不得读取 API Secret 或直连私流。桌面端无物理交易客户端、WAL、writer 或 artifacts 写权限；完整交易安全边界见 `ACCOUNT_MANAGEMENT.md` |
