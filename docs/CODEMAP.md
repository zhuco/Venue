...
| Control 服务核心 | `apps/venue-control/src/lib.rs` | 既有 Control HTTP/SSE、账户密文和旧 Copy 代码保持可编译。`0017_kol_copy_mvp.sql` 已增加最多5个启用KOL、200个启用关系、不可换绑归属、跨用户凭证/账户外键、源成交/目标及统一 Binance 命令账本；`kol_mvp.rs` 提供独占数据库会话持有的全局 Executor advisory lock。HTTP repository、真实 Executor binary 和页面仍属 P1–P3；旧 `venue-copy-worker` 不进入 MVP 发布链 |
