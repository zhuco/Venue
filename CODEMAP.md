...
| 本机构建资源约束 | `scripts/BUILD_POLICY.md` | `Invoke-VenueBuild.ps1` 与 `venue_build_guard.ps1` 管理 main/slot-1/slot-2、最多2个受控构建、150 GiB缓存准入及F/G空闲检查；专项脚本持锁到验证完成，不自动删除缓存 |
...
| Binance Portfolio Margin adapter | `crates/venue-gateway-binance/src/lib.rs` | `transport.rs`、`private_ws.rs`、`readback.rs` 与 `execution.rs` 提供绑定型 async HTTP/私流、Net/Hedge 完整腿、regular/Algo/conditional-unsupported、fills cursor、place/cancel/reduce-once 及 ACK 后 exact signed readback；`recovery.rs` 的 scope/raw/Owner/Unknown 六面 collector 仅作为测试夹具，生产不导出；Stage 7 capability/WAL/writer 仍是生产权威，adapter 静态能力为空 |
| Bitget adapter | `crates/venue-gateway-bitget/src/lib.rs` | `transport.rs`、`private_ws.rs`、`execution.rs` 与 `order_families.rs` 提供精确 TEST Demo/LIVE async 私有链路、账户五面同 attempt、normal/unsupported 订单族、place/cancel/reduce-once 与 UNKNOWN exact readback；`recovery.rs` 的完整候选只在测试构建中存在，生产 collector 固定 Unavailable；Stage 7 capability/WAL/writer 仍是生产权威，adapter 静态能力为空 |
| Gate.io adapter | `crates/venue-gateway-gate/src/lib.rs` | `transport.rs`、`private_surface.rs` 与 `execution.rs` 提供 TEST/LIVE async 签名 HTTP/私流、账户/Hedge 双腿、regular/profile-explicit-unsupported、fills cursor、post-only place/exact cancel/reduce-once 及 ACK 后 readback；`recovery.rs` 生产入口固定 Unavailable，scope/raw 构造仅测试可用；Stage 7 capability/WAL/writer 仍是生产权威，adapter 静态能力为空 |
...
| 六所固定账户节点产物 | `apps/venue-node/src/lib.rs` | 六 binary 继续逐 adapter 隔离；`async_gateway.rs` 保留单 runtime、线性、限时与 UNKNOWN 禁重投契约，但正向 token/async mutation 仅在测试夹具中存在，生产 `dispatch_prepared` 会把 Prepared 耐久终结为 `host_admission_unavailable` 且物理调用为零。共享 runtime 已有不可构造的恢复 session 与完整 universe/root 契约，但六所生产 collector 尚未接收该 session，也没有耐久 adapter 完整重放后签发 refresh，入口统一 Unavailable；Stage 7 仍是唯一生产 writer |
...
| 版本化 Control 协议 | `crates/venue-control-protocol/src/lib.rs` | schema v2 DTO 固定 snapshot、event、command、receipt 与账户节点 delivery/claim/ACK/Unknown/Reconciled 的精确 binding、instance/config epoch 校验；策略与命令显式携带 `TEST | LIVE`；协议不授予 capability、writer、WAL 或交易权限 |
| Control 服务核心 | `apps/venue-control/src/lib.rs` | Control 命令、本地 HTTP/SSE `/v2`、PostgreSQL fencing delivery lease 及 `account_node_poll.rs` 的 bounded claim/ACK/receipt HTTP 路由已落地；repository 始终重验 scope/lease/sequence，重复 receipt 幂等且冲突关闭；Node 已有 bounded HTTP polling client 与 opaque journal adapter，但 Actor durable Applied authority 尚未接线，TEST Copy 已接入，LIVE Copy 在数据库访问前失败关闭 |
| VenueFlow 原生/Web 客户端 | `apps/venueflow/src/main.rs` | native 用 `Last-Event-ID`、Web 用 `after=` 恢复命名 `control` SSE；snapshot/SSE 健康分离，接收上限 2 MiB、退避封顶 5 秒；Pause/Stop/Flatten 均要求精确 scope 确认；两端只访问 Control API，不持久化 secret/命令/receipt，不直连交易所、数据库或 artifacts |
...
- 六所候选准入审计：`scripts/verify_gateway_candidate_contract.ps1` 构建/测试六个隔离 binary，并验证 TEST/LIVE root 隔离及缺证据时零工件失败关闭；矩阵中的 `not_reached` 和 `writer_enabled=false` 是尚未接线的真实结论，不构成实盘准入。
