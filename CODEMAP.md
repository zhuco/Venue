...
| OKX V5 adapter | `crates/venue-gateway-okx/src/lib.rs` | `capability.rs` 闭合 place/cancel/reduce-once、Net/Hedge 腿、三订单族、fills cursor、`tdMode/mgnMode`、私流 generation 与 ACK UNKNOWN 只读收敛；完整新鲜 probe 只生成非权威候选，Net mutation 因共享命令边界继续关闭，静态能力及 node/writer/WAL 仍关闭 |
...
| 六所固定账户节点产物 | `apps/venue-node/src/lib.rs` | 六 binary 继续逐 adapter 隔离；`safe_host.rs` 与 `supervision.rs` 在 root/WAL/Owner/writer metadata 及 hash-chain control log 恢复后才允许连接，持久应用 Pause/Resume/Stop/Flatten/Canary 并令 UNKNOWN 禁重投；宿主本身不授予 capability，六所 adapter 尚未接入该物理边界，固定节点继续失败关闭 |
