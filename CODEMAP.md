...
| Gate.io adapter | `crates/venue-gateway-gate/src/lib.rs` | `transport.rs`、`private_surface.rs` 与 `execution.rs` 提供 LIVE async 签名 HTTP/私流、账户/Hedge 双腿、regular/profile-explicit-unsupported、fills cursor、post-only place/exact cancel/reduce-once 及 ACK 后 readback；`recovery_session.rs` 只在四频道 ACK 后签发一次性只读 session，冻结凭证、generation、完整规则/cursor/六面 universe、deadline 与全局 pages/bytes，每次 HTTP 前后要求唯一 nonce exact Pong；`recovery.rs` 生产不接 caller Owner/root，订单仅为 structured Unknown。该 candidate 不授予 capability/WAL/writer，Stage 7 仍是生产权威 |
| Bybit V5 adapter | `crates/venue-gateway-bybit/src/lib.rs` | 生产只保留只读 probe/readback，snapshot 与 candidate flags 仅含 `READ_*`；`physical`、`execution`、mutation 签名/POST/dispatch 与 recovery authority receipt 全部只在 `cfg(test)`，且不存在可由 feature 复活的测试入口；生产 collector 固定 `IntegrationUnavailable`，静态能力及 Node/writer/WAL 关闭 |
| OKX V5 adapter | `crates/venue-gateway-okx/src/lib.rs` | 生产只导出固定 GET 的只读 transport 与只读 probe candidate；`physical`、`execution`、mutation builder/trait/签名/POST 均限 `cfg(test)`，fresh recovery collector 固定 `IntegrationUnavailable`；静态能力及 Node/writer/WAL 关闭 |
| Hyperliquid adapter | `crates/venue-gateway-hyperliquid/src/lib.rs` | 生产只开放绑定 `/info` 的只读 candidate，fresh recovery collector 固定 `RecoveryIntegrationUnavailable`；凭证契约为主账户地址、API Wallet 地址及其私钥三项必填，Vault/Subaccount 地址可选且由是否存在表达，不使用额外布尔值或 Agent 名称；三类 action、签名、`/exchange` POST 与 physical dispatch 全部只在 `cfg(test)`；静态能力及 Node/writer/WAL 关闭 |
| CLI 定义与命令分派 | `src/cli.rs` | `src/app.rs`、`src/main.rs` |
| 六所固定账户节点产物 | `apps/venue-node/src/lib.rs` | 六 binary 继续逐 adapter 隔离；`async_gateway.rs` 保留单 runtime、线性、限时与 UNKNOWN 禁重投契约，但正向 token/async mutation 仅在测试夹具中存在，生产 `dispatch_prepared` 会把 Prepared 耐久终结为 `host_admission_unavailable` 且物理调用为零。Binance、Gate.io、Bitget 已有 adapter-owned authenticated 只读 collection session，但尚未接收共享 runtime 的完整 universe/root session，也没有耐久 replay 后签发 refresh；Bybit、OKX、Hyperliquid collector 仍统一 Unavailable。六所 Node physical installer/Ready/dispatch 继续关闭，Stage 7 仍是唯一生产 writer |
| Control→Node delivery inbox | `apps/venue-node/src/control_delivery.rs` | `control_delivery_storage.rs` 是 `venue_storage::OpaqueJournal` 的唯一 Node adapter，`control_http_client.rs` 只访问精确 loopback HTTP；claim、ACK 与 receipt 每次 HTTP await 后都以当前时钟重验 lease/session/epoch，ACK 绑定完整 inbox replay root/sequence/node，过期 outbox 不确认或重放并等待下一 epoch reconciliation。`venue-storage` 已有 Actor applied 双工件 durability receipt，但尚未由 runtime 以规范 Actor/Owner 和真实 WAL head 接线，生产 Applied 仍明确 Unavailable；不授予 capability、writer、WAL 或 dispatch |
| Actor Applied 持久化证据 | `crates/venue-storage/src/actor_applied.rs` | `create_new` 与 `open_existing(anchor)` 耦合 hash-chain journal 和原子 checkpoint；anchor 精确绑定 root/tail/count/checkpoint，缺件、截断、旧一致副本、崩溃窗口、scope/generation/turn/replay/WAL 回退均失败关闭。receipt 只证明调用方声明已耐久，不是身份、WAL 或 mutation authority；生产接线必须由 runtime 提供规范 Actor/Owner commitment 与真实 WAL durable head |
| 多策略账户运行时内核 | `crates/venue-runtime/src/account/mod.rs` | `authority.rs`、`registry.rs`、`private_router.rs`、`market_hub.rs`、`reconciler.rs` 与 `recovery.rs` 固定账户恢复/路由边界；`recovery_session.rs` 以私有 issuer seal 签发有界 session，绑定完整 registry universe/config epoch、position/profile、connection/private generation、五类 journal/checkpoint root、Owner/WAL/Unknown 与运行时 authority-state commitment，并要求每次 await 重验、至少一次 post-await refresh 后才可 install。生产 refresh 构造保持封闭，六所 collector 与耐久 replay adapter 尚未接线，因此 installer、Ready 与 Actor turn 仍统一 `PhysicalRecoveryIntegrationUnavailable`，仅测试显式启用 fixture |
...
| 版本化 Control 协议 | `crates/venue-control-protocol/src/lib.rs` | schema v2 DTO 固定 snapshot、event、command、receipt 与账户节点 delivery/claim/ACK/Unknown/Reconciled 的精确 LIVE binding、instance/config epoch 校验；协议不授予 capability、writer、WAL 或交易权限 |
| Control 服务核心 | `apps/venue-control/src/lib.rs` | Control 命令、本地 HTTP/SSE `/v2`、PostgreSQL fencing delivery lease 及 `account_node_poll.rs` 的 bounded claim/ACK/receipt HTTP 路由已落地；repository 始终重验 LIVE scope/lease/sequence，重复 receipt 幂等且冲突关闭；`venue-copy-worker` 只生成语义 job，mutation authority 恒为 false；Node 已有 bounded HTTP polling client 与 opaque journal adapter，但 Actor durable Applied authority 尚未接线 |
| VenueFlow 原生/Web 客户端 | `apps/venueflow/src/main.rs` | native 用 `Last-Event-ID`、Web 用 `after=` 恢复命名 `control` SSE；snapshot/SSE 健康分离，接收上限 2 MiB、退避封顶 5 秒；Pause/Stop/Flatten 均要求精确 scope 确认。原生端另以 `market_client.rs` 通过无凭证 Binance LIVE 公共 REST/WS 提供选中交易对的 K 线/BBO/成交/depth、全市场轻量 24h ticker、居中搜索器和顶部收藏标签，并以 `venue-indicators::chart` 从规范 PublicBar 计算图表指标；Web 保持 Control-only；两端均无账户私流、交易客户端、secret 或 artifacts 写权限 |
...
| 耐久 Owner/native identity | `crates/venue-execution/src/owner_routes.rs` | `DurableOwnerRoutes` 独占并重放同一 `CommandJournal`，按 command、Owner、family 与 native identity 恢复 route，cancel 只能取得原 create 的精确路由；不增加第二 journal、writer、capability 或 dispatch authority，尚未接入 Node 启动组合 |
...
| 指标与 FeatureFrame | `crates/venue-indicators/src/feature_frame.rs` | `chart/` 提供从 VenuePulse 行为迁移、基于规范已收盘 PublicBar 的 SMA/EMA/Bollinger/VWAP/RSI/MACD/ATR、静态注册表和 forming 克隆预览引擎；`public_book.rs`、`public_market_source.rs`、`scalping_features.rs`；根 `src/indicator/mod.rs` 仅适配根 OrderBook 并重导出 |
