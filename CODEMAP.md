...
| Bitget adapter | `crates/venue-gateway-bitget/src/lib.rs` | `config.rs` 将 TEST 精确映射 UTA v3 Demo（REST `paptrading: 1`、WS `wspap`），LIVE 映射生产；`credentials.rs` 隔离三项 secret 并拒绝 passphrase 别名冲突，`sign.rs` 固定 REST/WS HMAC-SHA256+Base64；`public.rs` 负责公共订阅、原始证据、订单簿序列与市场事件规范化，`risk.rs` 唯一负责 UTA 账户/腿风险规范化；根路径仅保留兼容入口，transport/private readback/Stage 7 writer 仍留根且不授予 mutation |
| Gate.io adapter | `crates/venue-gateway-gate/src/lib.rs` | `config.rs` 使用当前官方 Gate TEST/LIVE USDT futures REST/WS 端点；`credentials.rs` 隔离 key/secret，`sign.rs` 固定 REST 与私有订阅 HMAC-SHA512；`public.rs` 负责公共订阅、验签、规则与市场事件规范化，`risk.rs` 唯一负责合约数量、账户模式及账户/腿风险规范化；`GateGatewayBinding` 与产品 scope 不发行 capability 或 mutation authority，transport/private readback/Stage 7 writer 仍留根 package |
