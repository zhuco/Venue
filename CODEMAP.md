...
| 指标与 FeatureFrame | `crates/venue-indicators/src/feature_frame.rs` | `chart/` 提供从 VenuePulse 行为迁移、基于规范已收盘 PublicBar 的 SMA/EMA/Bollinger/VWAP/RSI/MACD/ATR、静态注册表和 forming 克隆预览引擎；`orderbook.rs` 持有纯 `OrderBook`/`PublicBook` 实现，另有 `public_book.rs`、`public_market_source.rs`、`scalping_features.rs`；根 `src/indicator/mod.rs` 只重导出 |
