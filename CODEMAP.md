...
| 六所网关身份、模式与能力门禁 | `crates/venue-gateway-api/src/lib.rs` | 规范 venue 固定 Binance、Bitget、Bybit、Gate.io、Hyperliquid、OKX；运行模式只接受精确 `TEST`、`LIVE`；`capability_promotion.rs` 将可持久化 probe 降为非权威候选，只有 Control applied、Owner/WAL 恢复、writer fence、Canary 和完整订单族证据同 scope/epoch/generation 后才生成不可序列化的进程内 `HostAdmittedCapability`；该 token 尚未接入 Node dispatch，静态能力仍为空 |
...
| 六所固定账户节点产物 | `apps/venue-node/src/lib.rs` | 六 binary 继续逐 adapter 隔离；`async_gateway.rs` 提供单 runtime、单 adapter、线性且限时的 sync→async 候选边界，断线/超时一律按 UNKNOWN 且不重投。部分 `run()` 仅实例化 inert candidate 做隔离/失败关闭预检；没有 fixed binary 构造 `TokioPhysicalGateway` 或把候选接入 `NodeSafetyHost`/dispatch，该边界仍用原始 snapshot 预检而未消费 `HostAdmittedCapability`。Stage 7 仍是唯一生产 writer |
| Control→Node delivery inbox | `apps/venue-node/src/control_delivery.rs` | 纯语义状态机以注入的 `ControlDeliveryJournal` 契约恢复 opaque record；`crates/venue-storage/src/control_delivery.rs` 已提供共享 hash-chain、排他 sequence fence、`sync_data` 与父目录同步的 `OpaqueJournal`。Node 尚未编写该契约的 storage adapter 或 HTTP polling client，不授予 capability、writer、WAL 或 dispatch |
| 多策略账户运行时内核 | `crates/venue-runtime/src/account/mod.rs` | `authority.rs`、`registry.rs`、`private_router.rs`、`market_hub.rs`、`reconciler.rs` 与 `recovery.rs` 固定账户恢复/路由边界；`physical_recovery.rs` 的内存值类型/验证器要求一个 attempt/new generation 完整覆盖账户、持仓、三订单族和成交游标六面，并绑定 Owner/WAL/Unknown roots，显式空账户也不得省略；尚未被 adapter、startup 或 binary 消费 |
