...
| Bitget adapter | `crates/venue-gateway-bitget/src/lib.rs` | `config.rs` 将 TEST 精确映射 UTA v3 Demo（REST `paptrading: 1`、WS `wspap`），LIVE 映射生产；`credentials.rs` 隔离三项 secret 并拒绝 passphrase 别名冲突，`sign.rs` 固定 REST/WS HMAC-SHA256+Base64；`public.rs` 负责公共订阅、原始证据、订单簿序列与市场事件规范化，`account.rs` 负责账户/模式/持仓，`risk.rs` 唯一负责 UTA 账户/腿风险规范化；根路径仅保留兼容入口，transport/private readback、订单/成交、Stage 7 writer 仍留根且不授予 mutation |
| Gate.io adapter | `crates/venue-gateway-gate/src/lib.rs` | `config.rs` 使用当前官方 Gate TEST/LIVE USDT futures REST/WS 端点；`credentials.rs` 隔离 key/secret，`sign.rs` 固定 REST 与私有订阅 HMAC-SHA512；`public.rs` 负责公共订阅、验签、规则与市场事件规范化，`private.rs` 负责账户/持仓，`risk.rs` 唯一负责合约数量、账户模式及账户/腿风险规范化；`GateGatewayBinding` 与产品 scope 不发行 capability 或 mutation authority，transport/private readback、订单/成交、Stage 7 writer 仍留根 package |
| Bybit V5 adapter | `crates/venue-gateway-bybit/src/lib.rs` | `binding.rs` 固定 Bybit/account/symbol/mode，TEST/LIVE 端点只能由 binding 派生；`credentials.rs` 与签名 header 使用 `secrecy`，`sign.rs` 同时核对完整请求 binding 并固定 V5 HMAC 原文；`models.rs` 与成交 fixture 保持原生 ID、client ID、Decimal 与未知 position side；capability 显式为空，尚未接 transport/private stream/mutation writer |
| OKX V5 adapter | `crates/venue-gateway-okx/src/lib.rs` | `binding.rs` 固定 OKX/account/symbol/mode，`config.rs` 由 binding 将 TEST 精确映射到模拟盘 WS/请求头、LIVE 映射生产端点；三项 secret 与签名 header 使用 `secrecy`，`sign.rs` 固定 REST 预哈希；成交 fixture 按已审核合约面值转成规范数量并核对账户持仓模式；capability 显式为空，尚未接 transport/private stream/mutation writer |
| Hyperliquid adapter | `crates/venue-gateway-hyperliquid/src/lib.rs` | `binding.rs` 固定 Hyperliquid/account/symbol/mode，`config.rs` 只能从 binding 派生 TEST testnet 或 LIVE mainnet；`credentials.rs` 在校验前即用 `secrecy` 接管命名 Agent/API Wallet 私钥并拒绝 owner/user/vault 与 Agent 冲突；`nonce.rs` 定义按 Agent 绑定、先持久化再签名的单调 checkpoint；私有成交 fixture 保留稳定身份与未知 position side；EIP-712 依赖尚未获准，signing/transport/writer 与 capability 均失败关闭 |
...
| 跟单纯规划内核 | `crates/venue-copy/src/lib.rs` | `capital.rs` 只消费单代、单估值币种且未过期的冻结资本事实；effective capital 取 configured、allocated、扣安全储备后 available margin 的最小值，复用 leader 已含杠杆的 exposure ratio 并扣除 managed exposure，跨零反向必须分两轮确认；`identity.rs` 精确保留 KOL 的 length-delimited SHA-256、UUID v5 shape、domain separation 与 job/snapshot/child/idempotency 固定身份；不持有 storage/network/runtime/writer/mutation authority；完整 sizing 待规范 Instrument 补齐合约换算语义后实现 |
