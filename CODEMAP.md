...
| Bybit V5 adapter | `crates/venue-gateway-bybit/src/account_gateway.rs`、`account_gateway_tests.rs` | 生产固定 UTA2/双向持仓预检、实时 linear 规则、policy-exact limit place/readback、精确 cancel 与 signed exact readback；原始 mutation transport 为 crate-private，只有 `AccountMutationHost` 的一次性 permit 可调用；账户入口的专项测试拆至同 crate 测试文件；市价、止损和本路径 reduce-only 暂不开放 |
...
| 限价执行政策 | `crates/venue-domain/src/domain/command.rs` | `LimitTimeInForce::{PostOnly, Gtc}` 为不可变命令字段；PostOnly 保留历史省略编码和 WAL hash，Gtc 显式编码。六 adapter 负责原生映射及即时/重启签名回读，`account_snapshot.rs`、`account_host.rs`、Runtime `reconciler.rs` 与 Node Grid/Copy 保留并精确比较政策；签名字段缺失保持未知。现有自动归一化仍只挂单；手动选价执行闭环未接通，Node Trade 继续明确拒绝 |
