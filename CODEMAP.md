...
| Binance Portfolio Margin adapter | `crates/venue-gateway-binance/src/lib.rs` | `binding.rs` 唯一定义 `PortfolioMarginUm`、原生 symbol 与 Binance `GatewayBinding` 的 TEST/LIVE 身份校验；`private.rs`、`portfolio.rs` 已物理迁入纯 payload 解析、订单族/成交/账户/仓位与风险规范化，根 `exchange::binance_private`、`binance_portfolio` 仅保持兼容 re-export；现有 transport、签名 readback、Stage 7 capability/WAL/writer 仍留根 package 且不从 GatewayBinding 推导 mutation authority |
| Bitget public adapter | `crates/venue-gateway-bitget/src/lib.rs` | `public.rs` 已物理迁入 UTA 公共订阅、原始证据、订单簿序列与市场事件规范化，根 `exchange::bitget_public` 仅保持兼容 re-export；`BitgetAccountBinding` 唯一定义 UTA hedge 产品身份且只校验 Bitget TEST/LIVE，不发行 capability 或 mutation authority；现有 HTTP/WS transport、私有 readback、Stage 7 capability/WAL/writer 仍留根 package |
| Gate.io public adapter | `crates/venue-gateway-gate/src/lib.rs` | `public.rs` 已物理迁入无凭证的公共订阅、验签、规则与市场事件规范化，根 `exchange::gate_public` 仅保持兼容 re-export；`GateGatewayBinding` 只校验 Gate.io TEST/LIVE 身份，不发行 capability 或 mutation authority；现有 HTTP/WS transport、私有 readback、Stage 7 capability/WAL/writer 仍留根 package |
