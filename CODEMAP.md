...
| VenueFlow 原生/Web 客户端 | `apps/venueflow/src/main.rs` | native 用 `Last-Event-ID`、Web 用 `after=` 恢复命名 `control` SSE；snapshot/SSE 健康分离，接收上限 2 MiB、退避封顶 5 秒；Pause/Stop/Flatten 均要求精确 scope 确认。原生端另以 `market_client.rs` 通过无凭证 Binance LIVE 公共 REST/WS 提供本地 K 线/BBO/成交/depth，Web 保持 Control-only；两端均无账户私流、交易客户端、secret 或 artifacts 写权限 |
