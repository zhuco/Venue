...
| 六所网关身份、模式与能力门禁 | `crates/venue-gateway-api/src/lib.rs` | 规范 venue 固定 Binance、Bitget、Bybit、Gate.io、Hyperliquid、OKX；运行模式只接受精确 `TEST`、`LIVE`；`capability_promotion.rs` 定义同 scope/epoch/generation 的 Control、Owner/WAL、writer、Canary 与订单族候选组合，但其上游 receipt 仍可由普通调用方构造，不能作为生产 authority；Node 生产 dispatch 因此固定拒绝，静态能力仍为空 |
...
| Hyperliquid adapter | `crates/venue-gateway-hyperliquid/src/lib.rs` | 生产只开放绑定 `/info` 的只读 candidate，fresh recovery collector 固定 `RecoveryIntegrationUnavailable`；三类 action、签名、`/exchange` POST 与 physical dispatch 全部只在 `cfg(test)`；静态能力及 Node/writer/WAL 关闭 |
| CLI 定义与命令分派 | `src/cli.rs` | `src/app.rs`、`src/main.rs` |
| 六所固定账户节点产物 | `apps/venue-node/src/lib.rs` | 六 binary 继续逐 adapter 隔离；`async_gateway.rs` 保留单 runtime、线性、限时与 UNKNOWN 禁重投契约，但正向 token/async mutation 仅在测试夹具中存在，生产 `dispatch_prepared` 会把 Prepared 耐久终结为 `host_admission_unavailable` 且物理调用为零。六所已有 scope/raw/Owner/结构化 Unknown 的测试 collector，但生产尚无不可伪造认证会话、完整账户 universe 与耐久 root authority，入口统一 Unavailable；Stage 7 仍是唯一生产 writer |
| Control→Node delivery inbox | `apps/venue-node/src/control_delivery.rs` | `control_delivery_storage.rs` 是 `venue_storage::OpaqueJournal` 的唯一 Node adapter，`control_http_client.rs` 只访问精确 loopback HTTP；claim、ACK 与 receipt 每次 HTTP await 后都以当前时钟重验 lease/session/epoch，ACK 绑定完整 inbox replay root/sequence/node，过期 outbox 不确认或重放并等待下一 epoch reconciliation。当前缺少不可伪造的 Actor durable-applied authority，生产 Applied receipt 明确 Unavailable；不授予 capability、writer、WAL 或 dispatch |
| 多策略账户运行时内核 | `crates/venue-runtime/src/account/mod.rs` | `authority.rs`、`registry.rs`、`private_router.rs`、`market_hub.rs`、`reconciler.rs` 与 `recovery.rs` 固定账户恢复/路由边界；`physical_recovery.rs` 定义证据值类型，`physical_recovery_runtime.rs` 负责安装、漂移撤权与 runtime 组合，绑定 mode/account/native binding、完整 registry universe/config epoch、position mode、profile version、connection/private generation 与 Owner/WAL/structured Unknown roots。生产 collector 仍缺 opaque 认证会话与耐久刷新 authority，因此 installer、Ready 与 Actor turn 统一 `PhysicalRecoveryIntegrationUnavailable`，仅测试显式启用 fixture |
