...
| Bybit V5 adapter | `crates/venue-gateway-bybit/src/lib.rs` | `evidence.rs` 闭合账户、Hedge 双腿、regular/conditional 签名分页及绑定型 Algo unsupported；成交经完整订单详情重放，ACK 后只做 exact lookup，超时/断线不重投；`transport.rs` 保持限时限长、ACK 前有界缓存、心跳与 generation；仍无 writer/WAL/capability |
| OKX V5 adapter | `crates/venue-gateway-okx/src/lib.rs` | Net/Hedge 完整腿、三订单族、fills cursor、`tdMode/mgnMode`、私流 generation 与 ACK 原文 hash/readback 均绑定账户/symbol/mode；超时后只签名读回、不重投；`transport.rs` 保持限时限长、缓存与心跳；仍无 writer/WAL/capability |
| Hyperliquid adapter | `crates/venue-gateway-hyperliquid/src/lib.rs` | `/info`、恢复查询与双私流 generation 已绑定；`action.rs` 提供 ALO/cancel/IOC reduce-only、签名向量、持久 nonce 与 ACK 后 orderStatus 收敛，UNKNOWN 禁重投；成交历史明确 2000/页、最近 10000 条边界并跨频道去重；`tid` 不是 sequence；writer/WAL/capability 仍为空 |
| CLI 定义与命令分派 | `src/cli.rs` | `src/app.rs`、`src/main.rs` |
| 六所固定账户节点产物 | `apps/venue-node/src/lib.rs` | 六 binary 继续逐 adapter 隔离；`safe_host.rs` 用窄 `PhysicalGateway` 组合 canonical root、唯一 writer、Owner、WAL、一次性 permit、UNKNOWN 读回、Stop/Flatten 与 command-bound Canary，但本身不授予 capability；前三所 LIVE 仍委托 Stage 7、TEST 仍失败关闭；新增三所尚未把 adapter/capability 接入安全宿主，固定节点继续在读凭证、联网、建工件前失败关闭 |
...
| Control 服务核心 | `apps/venue-control/src/lib.rs` | Control 命令及本地 HTTP/SSE `/v2` 已落 PostgreSQL；`copy_worker.rs` 与 migration 0003 提供 TEST-only worker，事务内持久冻结资本、目标、immutable manifest、job/cursor，处理四类 receipt、Unknown 禁重投、ledger 与恢复；LIVE 在数据库访问前失败关闭；仍无账户节点 adapter、交易客户端、writer/WAL |
