...
| 多策略账户运行时内核 | `crates/venue-runtime/src/account/mod.rs` | `authority.rs`、`registry.rs`、`private_router.rs`、`market_hub.rs`、`reconciler.rs` 与 `recovery.rs` 固定账户恢复/路由边界；`physical_recovery.rs` 绑定 mode/account/native binding、完整 registry universe/config epoch、position mode、profile version、connection/private generation 与 Owner/WAL/structured Unknown roots；任一 WAL/Unknown/root/config/profile 漂移立即撤销 Ready/turn。生产 collector 仍缺 opaque 认证会话与耐久刷新 authority，因此 installer、Ready 与 Actor turn 统一 `PhysicalRecoveryIntegrationUnavailable`，仅测试显式启用 fixture |
