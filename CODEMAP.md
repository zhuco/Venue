...
| 功能 | 首要入口 | 直接继续 |
|---|---|---|
| 构建、依赖与仓库体积门禁 | `Cargo.toml` | workspace 当前包含根 package、`venue-copy`、`venue-domain`、`venue-gateway-api`、`venue-gateway-bybit`、`venue-gateway-hyperliquid` 与 `venue-gateway-okx`，resolver 固定为 3；只在真实代码迁移时增加成员；workspace 与 `rust-toolchain.toml` 共同锁定 Rust 1.98.0；`Cargo.lock`；`scripts/verify_repository_hygiene.ps1` 拒绝跟踪运行态/构建/发布/secret 文件、单文件超过 2 MiB 或工作树总量超过 10 MiB |
