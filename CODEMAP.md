...
| Binance Portfolio Margin adapter | `crates/venue-gateway-binance/src/lib.rs` | `transport.rs`、`private_ws.rs`、`readback.rs` 与 `execution.rs` 提供绑定型 async HTTP/私流、Net/Hedge 完整腿、regular/Algo/conditional-unsupported、fills cursor、place/cancel/reduce-once 及 ACK 后 exact signed readback；UNKNOWN 禁重投；Stage 7 capability/WAL/writer 仍是生产权威，adapter 静态能力为空 |
| Bitget adapter | `crates/venue-gateway-bitget/src/lib.rs` | `transport.rs`、`private_ws.rs`、`execution.rs` 与 `order_families.rs` 提供精确 TEST Demo/LIVE async 私有链路、账户五面同 attempt、normal/unsupported 订单族、place/cancel/reduce-once 与 UNKNOWN exact readback；Stage 7 capability/WAL/writer 仍是生产权威，adapter 静态能力为空 |
| Gate.io adapter | `crates/venue-gateway-gate/src/lib.rs` | `transport.rs`、`private_surface.rs` 与 `execution.rs` 提供 TEST/LIVE async 签名 HTTP/私流、账户/Hedge 双腿、regular/profile-explicit-unsupported、fills cursor、post-only place/exact cancel/reduce-once 及 ACK 后 readback；Stage 7 capability/WAL/writer 仍是生产权威，adapter 静态能力为空 |
| Bybit V5 adapter | `crates/venue-gateway-bybit/src/lib.rs` | `physical.rs` 与 `evidence.rs` 闭合绑定型 place/cancel/reduce-only IOC、账户/Hedge 双腿、三订单族、订单详情/fills、私流 generation 和 ACK 后 exact lookup；capability probe 可持久化并以 API-secret HMAC 防重算篡改，静态能力及 node/writer/WAL 仍关闭 |
| OKX V5 adapter | `crates/venue-gateway-okx/src/lib.rs` | `capability.rs` 与 `physical.rs` 闭合 place/cancel/reduce-once、Net/Hedge 腿、三订单族、fills cursor、`tdMode/mgnMode`、私流 generation 与 ACK UNKNOWN 只读收敛；共享命令已能按权威 Net 持仓校验开/减仓，但当前 OKX 候选仍只对 Hedge mutation 建模；完整新鲜 probe 只生成非权威候选，静态能力及 node/writer/WAL 仍关闭 |
| Hyperliquid adapter | `crates/venue-gateway-hyperliquid/src/lib.rs` | `physical.rs` 与 `capability.rs` 提供单次 dispatch、ALO/cancel/IOC reduce-only、ACK 断线 UNKNOWN、orderStatus 收敛及绑定 vault/coin/mode/generation 的可持久化 probe；三类 action 与成交窗口承诺防篡改，静态能力及 node/writer/WAL 仍关闭 |
| CLI 定义与命令分派 | `src/cli.rs` | `src/app.rs`、`src/main.rs` |
| 六所固定账户节点产物 | `apps/venue-node/src/lib.rs` | 六 binary 继续逐 adapter 隔离；`async_gateway.rs` 保留单 runtime、线性、限时与 UNKNOWN 禁重投契约，但正向 token/async mutation 仅在测试夹具中存在，生产 `dispatch_prepared` 会把 Prepared 耐久终结为 `host_admission_unavailable` 且物理调用为零。六所六面映射也只保留测试契约，缺少 scope-bound fresh collector、结构化 Unknown 与 exact Owner 时生产入口统一 Unavailable；Stage 7 仍是唯一生产 writer |
| Control→Node delivery inbox | `apps/venue-node/src/control_delivery.rs` | `control_delivery_storage.rs` 是 `venue_storage::OpaqueJournal` 的唯一 Node adapter，`control_http_client.rs` 只访问精确 loopback HTTP 并限制连接/请求时限和请求/响应大小；顺序固定 claim→本地耐久 append→ACK→Actor→receipt，过期 turn 只跳过自身并等待下一 epoch reconciliation。当前缺少不可伪造的 Actor durable-applied authority，生产 Applied receipt 明确 Unavailable；不授予 capability、writer、WAL 或 dispatch |
| 多策略账户运行时内核 | `crates/venue-runtime/src/account/mod.rs` | `authority.rs`、`registry.rs`、`private_router.rs`、`market_hub.rs`、`reconciler.rs` 与 `recovery.rs` 固定账户恢复/路由边界；`physical_recovery.rs` 的值类型要求 connection generation、同一 attempt/new private generation、账户/持仓/三订单族/FillsCursor 六面及 Owner/WAL/Unknown roots。由于尚无 opaque fresh collector、多 symbol 完整性和运行期 root refresh authority，生产 installer、Ready 与 Actor turn 统一 `PhysicalRecoveryIntegrationUnavailable`，仅测试显式启用 fixture |
...
| Control 服务核心 | `apps/venue-control/src/lib.rs` | Control 命令、本地 HTTP/SSE `/v2`、PostgreSQL fencing delivery lease 及 `account_node_poll.rs` 的 bounded claim/ACK/receipt HTTP 路由已落地；repository 始终重验 scope/lease/sequence，重复 receipt 幂等且冲突关闭；Node HTTP polling client 尚不存在，TEST Copy 已接入，LIVE Copy 在数据库访问前失败关闭 |
