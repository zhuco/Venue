...
| 功能 | 首要入口 | 直接继续 |
|---|---|---|
| 构建、依赖与仓库体积门禁 | `Cargo.toml` | workspace 当前包含根 package、`venue-copy`、`venue-domain`、`venue-gateway-api`、`venue-gateway-binance`、`venue-gateway-bitget`、`venue-gateway-bybit`、`venue-gateway-gate`、`venue-gateway-hyperliquid` 与 `venue-gateway-okx`，resolver 固定为 3；只在真实代码迁移时增加成员；workspace 与 `rust-toolchain.toml` 共同锁定 Rust 1.98.0；`Cargo.lock`；`scripts/verify_repository_hygiene.ps1` 拒绝跟踪运行态/构建/发布/secret 文件、单文件超过 2 MiB 或工作树总量超过 10 MiB |
...
| Binance Portfolio Margin adapter | `crates/venue-gateway-binance/src/lib.rs` | `binding.rs` 唯一定义 `PortfolioMarginUm`、原生 symbol 与 Binance `GatewayBinding` 的 TEST/LIVE 身份校验；`private.rs`、`portfolio.rs` 已物理迁入纯 payload 解析、订单族/成交/账户/仓位与风险规范化，`fill_pagination.rs` 归一七日窗口、单调 cursor 与有界成交分页；根路径仅兼容 re-export，现有 transport、签名 readback、Stage 7 capability/WAL/writer 仍留根 package且不从 GatewayBinding 推导 mutation authority |
| Bitget public adapter | `crates/venue-gateway-bitget/src/lib.rs` | `config.rs` 将 TEST 精确映射 UTA v3 Demo（REST `paptrading: 1`、WS `wspap`），LIVE 映射生产；`credentials.rs` 隔离三项 secret 并拒绝 passphrase 别名冲突，`sign.rs` 固定 REST/WS HMAC-SHA256+Base64；`public.rs` 已物理迁入公共订阅、原始证据、订单簿序列与市场事件规范化；根路径仅兼容 re-export，transport/private/Stage 7 writer 仍留根且不授予 mutation |
| Gate.io public adapter | `crates/venue-gateway-gate/src/lib.rs` | `config.rs` 使用当前官方 Gate TEST/LIVE USDT futures REST/WS 端点；`credentials.rs` 隔离 key/secret，`sign.rs` 固定 REST 与私有订阅 HMAC-SHA512；`public.rs` 已物理迁入公共订阅、验签、规则与市场事件规范化；`GateGatewayBinding` 与产品 scope 不发行 capability 或 mutation authority，transport/private/Stage 7 writer 仍留根 package |
| Bybit V5 adapter | `crates/venue-gateway-bybit/src/lib.rs` | `config.rs` 只映射 TEST testnet 与 LIVE production；`credentials.rs` 使用 `secrecy`；`sign.rs` 固定 V5 HMAC 原文；`models.rs` 与 `fixtures/execution-trade-page.json` 提取 KOL 签名成交 fixture 并保持原生 ID、client ID、Decimal 与未知 position side；尚未接 transport/private stream/mutation writer，账户能力仍为空 |
| OKX V5 adapter | `crates/venue-gateway-okx/src/lib.rs` | `config.rs` 将 TEST 精确映射到模拟盘 WS/请求头、LIVE 映射生产端点；`credentials.rs` 隔离三项 secret；`sign.rs` 固定 REST 预哈希与模拟盘头；成交 fixture 按已审核合约面值转成规范数量并核对账户持仓模式；尚未接 transport/private stream/mutation writer，账户能力仍为空 |
| Hyperliquid adapter | `crates/venue-gateway-hyperliquid/src/lib.rs` | `config.rs` 只映射 TEST testnet 与 LIVE mainnet；`credentials.rs` 在校验前即用 `secrecy` 接管命名 Agent/API Wallet 私钥并拒绝把 owner 地址声明为 Agent；`nonce.rs` 定义按 Agent 绑定、先持久化再签名的单调 checkpoint；私有成交 fixture 用 `账户地址 + native coin + tid` 建立透明稳定身份，保留 client ID、Decimal 与来源未给出的 position side；EIP-712 签名依赖尚未获准，signing/transport/writer 与账户 capability 均失败关闭 |
...
| 跟单目标敞口纯 reducer | `crates/venue-copy/src/capital.rs` | 只消费单代、单估值币种且未过期的冻结资本事实；effective capital 取 configured、allocated、扣安全储备后 available margin 的最小值，复用 leader 已含杠杆的 exposure ratio 并扣除 managed exposure；跨零反向必须分两轮确认；不持有 storage/network/runtime/writer/mutation authority；完整 sizing 待规范 Instrument 补齐合约换算语义后实现 |
...
| 规范交易账户、交易对、金额、订单、仓位、成交 | `crates/venue-domain/src/domain/mod.rs` | `identity.rs` 唯一定义规范账户 UUID 校验；其余为 `crates/venue-domain/src/domain/{symbol,money,order,position,event,instrument}.rs`；根 `src/domain/mod.rs` 保持既有 `venue::domain` facade，并暂留构造器受 crate 权限保护的 runtime identity，禁止为跨 crate 迁移公开 turn authority |
...
| Gate USDT 永续 | `src/exchange/gate/mod.rs` | 公共纯协议位于 `crates/venue-gateway-gate/src/public.rs`，根 `exchange::gate_public` 兼容重导出；`src/exchange/gate/gate_risk.rs` 及 transport/private/Stage 7 保持生产权威；支持当前 Unified Single-Currency 权益；当前 Stage 7 profile 对常规族保存签名页，条件/Algo 明确不支持，因而无该两族 mutation 准入；风险减仓回报只有严格 `t-ord-etp-{l|s}-<16 小写 hex>` 才可归因 hedge side，其他不透明 text 保持未知 |
| Bitget UTA 永续 | `src/exchange/bitget/mod.rs` | 公共纯协议位于 `crates/venue-gateway-bitget/src/public.rs`，根 `exchange::bitget_public` 兼容重导出；`src/exchange/bitget/bitget_risk.rs` 及 transport/private/Stage 7 保持生产权威；当前 Stage 7 profile 保存 `delegateType=normal` 的常规族签名页，条件/Algo 明确不支持；账户/设置/持仓/订单/成交五面任一失败即作废整轮，禁止跨尝试拼成一代；精确终态订单只在方向化 `tradeSide` 与 position side/side 一致时接纳，成交时间优先 `execTime`；handoff 尚有风险减仓待结算时不得推进 Bitget 成交历史窗口 |
