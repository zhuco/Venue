...
| 六所固定账户节点产物 | `apps/venue-node/src/lib.rs` | 六 binary 继续逐 adapter 隔离；每个固定 binary 已在本地封装对应 adapter 的 `PhysicalGateway` 候选与契约测试，但未接入 `run()`/启动链，capability 仍为空；`safe_host.rs` 与 `supervision.rs` 在 root/WAL/Owner/writer metadata 及 hash-chain control log 恢复后才允许连接，持久应用 Pause/Resume/Stop/Flatten/Canary 并令 UNKNOWN 禁重投。现有 Stage 7 仍是唯一生产 writer，新候选继续失败关闭 |
| Control→Node delivery inbox | `apps/venue-node/src/control_delivery.rs` | 纯语义状态机以注入的 `ControlDeliveryJournal` 契约恢复 opaque record，并闭合 binding/instance/config epoch、lease fencing、耐久 ACK 后 Actor apply、Unknown 下一序号只读 reconciliation、幂等/冲突和崩溃恢复；当前仅有内存测试，尚未接入共享 journal 实现或 HTTP polling，也不授予 capability、writer、WAL 或 dispatch |
...
| Control 服务核心 | `apps/venue-control/src/lib.rs` | Control 命令、本地 HTTP/SSE `/v2` 与 PostgreSQL fencing delivery lease 已落地；migration 0004、`account_delivery_postgres.rs` 提供崩溃重领、耐久 node inbox ACK、Unknown 后只读 reconciliation claim 及重复 receipt 幂等/冲突检查；TEST Copy 已接入，LIVE Copy 仍在数据库访问前失败关闭；节点轮询 adapter 与交易权限仍不存在 |
