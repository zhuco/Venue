...
| Bybit V5 adapter | `crates/venue-gateway-bybit/src/account_gateway.rs` | 生产固定 UTA2/双向持仓预检、实时 linear 规则、post-only place、精确 cancel 与 signed exact readback；原始 mutation transport 为 crate-private，只有 `AccountMutationHost` 的一次性 permit 可调用；市价、止损和本路径 reduce-only 暂不开放 |
