...
function Get-VenueBuildPlan {
    ...
    return [PSCustomObject]@{RepoRoot=$repo;Root=$root;Slot=$Slot;TargetDirectory=$target;TempDirectory=(Join-Path (Join-Path $root '.tmp') $Slot);GuardDirectory=(Join-Path $root '.guard');HostedCI=$false;BudgetBytes=300GB;MinimumHostFree=100GB;MinimumGuestFree=20GB;HostRoot='F:\';GuestRoot='G:\'}
}
...
function Test-VenueBuildAdmission {
    ...
    if ($bytes -gt $Plan.BudgetBytes) { throw "Build refused: the $([math]::Ceiling($Plan.BudgetBytes / 1GB)) GiB cache admission budget is exceeded. Review idle registered caches; do not clean source or recovery data." }
