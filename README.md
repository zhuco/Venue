...
VENUE 提供 Binance 交易终端、KOL 限价挂单同步和对冲网格。VenueFlow 是原生桌面客户端，Web 提供邀请注册、账户管理和跟单设置；交易统一由服务端单个多账户 `venue-executor-binance` 执行。Binance KOL、终端和 Binance Grid 使用 Portfolio Margin UM 双向持仓账户；另外五所通过独立策略入口执行，支撑分批做多首个闭环使用 Bybit LIVE。
...
| KOL 挂单同步 | 同步新普通限价单及撤单，保留源价、方向和 GTC/PostOnly；每账户独立选择定比或定额 | 需带单授权、机器人运行及跟随关系各自启用；不复制市价或条件单，不追补仓位差异 |
