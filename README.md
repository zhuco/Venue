...
Control schema v2。当前目标是交易终端可用、真实跟单业务可用、旧三所接管：Binance 优先，再 Gate.io、Bitget；Bybit、OKX、Hyperliquid 与 Scalping 暂缓。六所有实现不等于已验收，本版本不是可无人值守上线的稳定版。
...
| Grid | 共享 reducer、Actor checkpoint、行情 FeatureSource | 旧三所私流/多 symbol 协同、恢复及实际接管 |
| Scalping | 保留既有实现与失败关闭 | 暂缓处理，不进入当前三目标验收 |
...
- [三目标执行契约与启动提示词](docs/UNIFIED_GATEWAY_WEB_MIGRATION.md)：当前任务的唯一目标/验收入口。
