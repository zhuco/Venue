...
mod tests {
    ...
    #[test]
    fn okx_binding_keeps_exact_account_symbol_and_test_or_live_mode()
    -> Result<(), Box<dyn std::error::Error>> {
        for mode in [GatewayMode::Test, GatewayMode::Live] {
            let binding = OkxGatewayBinding::new(binding(VenueId::Okx, mode)?)?;
            assert_eq!(binding.gateway_binding().venue, VenueId::Okx);
            assert_eq!(binding.gateway_binding().mode, mode);
            assert_eq!(
                binding.gateway_binding().trading_account_id,
                "00000000-0000-4000-8000-000000000001"
            );
            assert_eq!(binding.gateway_binding().symbol.to_string(), "BTC/USDT");
        }
        assert_eq!(
