...
#[serde(bound(deserialize = "S: DeserializeOwned, U: DeserializeOwned"))]
pub struct ScalpingRiskBinding<S, U> {
    pub exchange: String,
    pub account: String,
...
impl<S, U: RiskUnitValue> ScalpingRiskBinding<S, U> {
...
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
#[serde(bound(deserialize = "S: DeserializeOwned, U: DeserializeOwned, F: DeserializeOwned"))]
pub struct ScalpingRiskFact<S, U, F> {
    pub binding: ScalpingRiskBinding<S, U>,
    pub fact: F,
}

impl<S, U: RiskUnitValue, F: RiskFactValue<U>> ScalpingRiskFact<S, U, F> {
    fn validate(&self) -> Result<(), ScalpingRiskError> {
...
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
#[serde(bound(deserialize = "S: DeserializeOwned, U: DeserializeOwned"))]
pub struct ScalpingRiskCursor<S, U> {
...
impl<S, U: RiskUnitValue> ScalpingRiskCursor<S, U> {
    fn validate(&self) -> Result<(), ScalpingRiskError> {
        self.binding.validate()?;
...
#[serde(bound(deserialize = "S: DeserializeOwned, U: DeserializeOwned, F: DeserializeOwned"))]
pub enum ScalpingRiskEntry<S, U, F> {
    Fact(ScalpingRiskFact<S, U, F>),
    Cursor(ScalpingRiskCursor<S, U>),
}

#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
#[serde(bound(deserialize = "S: DeserializeOwned, U: DeserializeOwned, F: DeserializeOwned"))]
pub struct ScalpingRiskRecord<S, U, F> {
    pub sequence: u64,
    pub content_sha256: String,
    pub entry: ScalpingRiskEntry<S, U, F>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ScalpingRiskRecovery<S, U, F> {
    pub records: Vec<ScalpingRiskRecord<S, U, F>>,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ScalpingRiskReplay<S, U, F> {
    pub cursor: ScalpingRiskCursor<S, U>,
    pub cursor_sequence: u64,
    pub facts: Vec<ScalpingRiskFact<S, U, F>>,
}

#[derive(Debug)]
pub struct ScalpingRiskJournal<S, U, F> {
    path: PathBuf,
    next_sequence: u64,
    state: JournalState<S, U>,
    fact: std::marker::PhantomData<F>,
}

impl<S, U, F> ScalpingRiskJournal<S, U, F>
where
    S: Clone + Debug + Eq + Ord + Serialize + DeserializeOwned,
    U: RiskUnitValue,
    F: RiskFactValue<U>,
{
    pub fn open(path: impl Into<PathBuf>) -> Result<Self, ScalpingRiskError> {
        let path = path.into();
        let recovery: ScalpingRiskRecovery<S, U, F> = recover_file(&path)?;
    ...
    pub fn append_page(
        &mut self,
        facts: Vec<ScalpingRiskFact<S, U, F>>,
        cursor: ScalpingRiskCursor<S, U>,
    ) -> Result<ScalpingRiskCommit, ScalpingRiskError> {
        cursor.validate()?;
        let cursor_digest = digest_entry::<S, U, F>(&ScalpingRiskEntry::Cursor(cursor.clone()))?;
        ...
        let supplied_ids = facts
            .iter()
            .map(|fact| fact.fact.fact_id().to_owned())
            .collect::<BTreeSet<_>>();
        ...
        for fact in &facts {
            ...
            match supplied_digests.insert(fact.fact.fact_id().to_owned(), digest.clone()) {
    ...
    pub fn recover(&self) -> Result<ScalpingRiskRecovery<S, U, F>, ScalpingRiskError> {
    ...
    pub fn committed_replays(&self) -> Result<Vec<ScalpingRiskReplay<S, U, F>>, ScalpingRiskError> {
        self.recover_committed_replays()
    }

    pub fn recover_committed_replays(
        &self,
    ) -> Result<Vec<ScalpingRiskReplay<S, U, F>>, ScalpingRiskError> {
    ...
    fn append_fact(&mut self, fact: ScalpingRiskFact<S, U, F>) -> Result<u64, ScalpingRiskError> {
...
impl<S: Clone, U: Clone> ScalpingRiskBinding<S, U> {
    fn scope(&self) -> ScalpingRiskScope<S, U> {
        ScalpingRiskScope {
...
struct Identity<S, U> {
    digest: String,
    sequence: u64,
    binding: ScalpingRiskBinding<S, U>,
...
impl FactKey {
    fn from_fact<U, F: RiskFactValue<U>>(fact: &F) -> Self {
        Self {
    ...
    fn from_cursor<S, U>(binding: &ScalpingRiskBinding<S, U>, fact_id: &str) -> Self {
        Self {
...
#[derive(Debug)]
struct JournalState<S, U> {
    scope: Option<ScalpingRiskScope<S, U>>,
    fact_ids: BTreeMap<FactKey, Identity<S, U>>,
    cursor_ids: BTreeMap<String, Identity<S, U>>,
    last_cursors: BTreeMap<u64, ScalpingRiskCursor<S, U>>,
}

impl<S, U> Default for JournalState<S, U> {
    fn default() -> Self {
        Self {
            scope: None,
            fact_ids: BTreeMap::new(),
            cursor_ids: BTreeMap::new(),
            last_cursors: BTreeMap::new(),
        }
    }
}

impl<S, U> JournalState<S, U>
where
    S: Clone + Debug + Eq + Ord + Serialize + DeserializeOwned,
    U: RiskUnitValue,
{
    fn from_records<F: RiskFactValue<U>>(
        records: &[ScalpingRiskRecord<S, U, F>],
    ) -> Result<Self, ScalpingRiskError> {
    ...
    fn validate_scope(&self, binding: &ScalpingRiskBinding<S, U>) -> Result<(), ScalpingRiskError> {
        binding.validate()?;
    ...
    fn insert_fact<F: RiskFactValue<U>>(
        &mut self,
        fact: ScalpingRiskFact<S, U, F>,
        digest: String,
        sequence: u64,
        ...
        match self.fact_ids.get(&key) {
            ...
            None => {
                ...
                let event_time_ms = fact.fact.event_time_ms();
...
fn digest_entry<S: Serialize, U: Serialize, F: Serialize>(
    entry: &ScalpingRiskEntry<S, U, F>,
) -> Result<String, ScalpingRiskError> {
    let bytes = serde_json::to_vec(entry).map_err(ScalpingRiskError::Encode)?;
    Ok(format!("{:x}", Sha256::digest(bytes)))
}

fn recover_file<S, U, F>(path: &Path) -> Result<ScalpingRiskRecovery<S, U, F>, ScalpingRiskError>
where
    S: DeserializeOwned,
    U: DeserializeOwned,
    F: DeserializeOwned,
