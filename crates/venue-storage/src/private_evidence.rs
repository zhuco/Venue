...
mod tests {
    ...
    #[test]
    fn generation_and_durable_receipts_survive_reopen() -> Result<(), Box<dyn std::error::Error>> {
        ...
        journal.append(PrivateEvidence::new(9, 22, "second".to_owned())?)?;
        ...
        let reopened = PrivateEvidenceJournal::open(path)?;
        assert_eq!(reopened.last_generation(), 9);
        assert_eq!(reopened.recover_persisted()?[0], appended);
        Ok(())
    }
}
