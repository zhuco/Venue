        ...
        let prepared = match permit.command() {
            ...
            ExecutionCommand::Cancel(command) => {
                let target = self
                    ...
                    .orders
                    .iter()
                    .find(|order| matches!(&order.client_order_id, FieldState::Known(value) if value == command.target_client_order_id.as_str()))
...
fn snapshot_regular_order_facts(
    ...
    rows.iter()
        .map(|row| {
            ...
            let client_order_id = snapshot_text(item.get("text"))?.to_owned();
...
mod tests {
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert!(
            parse_fresh_limit_bbo(&rules, binding.clone(), wrong_symbol, 1_000_000, 1_000_200)
                .is_err()
        );
        ...
        assert!(
            parse_fresh_limit_bbo(&rules, binding.clone(), crossed, 1_000_000, 1_000_200).is_err()
        );
        let empty = BBO.replace("[{\"p\":\"0.1000\",\"s\":\"20\"}]", "[]");
        assert!(
            parse_fresh_limit_bbo(&rules, binding.clone(), empty, 1_000_000, 1_000_200).is_err()
        );
        assert!(
            parse_fresh_limit_bbo(
                &rules,
    ...
    #[test]
    fn signed_snapshot_normalizes_all_account_position_and_regular_order_facts()
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(order_facts[0].quantity, Decimal::new(5, 1));
    ...
    #[test]
    fn signed_snapshot_exports_normalized_account_fill_facts()
    -> Result<(), Box<dyn std::error::Error>> {
